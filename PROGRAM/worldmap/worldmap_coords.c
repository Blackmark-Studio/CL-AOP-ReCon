...
void CheckQuestCoordinates_GTutor()
{
	if (IsEntity(&worldMap)) // если на глобальной карте
	{
		bWMEncounterCheck = false;

		Log_TestInfo("Тек. остров : " + worldmap.island);
		if (worldmap.island == "Tortuga")
		{
		    wdmLockReload = false;
		    ILogAndActions.ActiveActions.EnterToIsland.Text	= XI_ConvertString("for_quick_action_EnterToSea");
...
string Map_GetRealCoordX(float X)
{
	...
	string sCoordX = iRDegX + " " + XI_ConvertString("grd") + " " + iRMinX + "' " + iRSecX + "'' " + XI_ConvertString("LongitudeShort");
	return sCoordX;
}
...
string Map_GetRealCoordZ(float Z)
{
	...
	string sCoordZ = iRDegZ + " " + XI_ConvertString("grd") + " " + iRMinZ + "' " + iRSecZ + "'' " + XI_ConvertString("LatitudeShort");
	return sCoordZ;
}
