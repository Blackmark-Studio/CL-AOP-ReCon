...
{
	float fMaterialH, fMaterialS;
	int chrIdx = GetEventData();
	int eRepType = GetEventData();
	
	if(eRepType!=0) // fix
	{
		fMaterialH = GetEventData();
		fMaterialS = GetEventData();
	}
	...
	//======================================================
	{
		...
		int iBonus = GetParamPageBonus("InstantRepair");
		if(hpp < InstantRepairRATE + iBonus && nMaterialH>0) // boal 23.01.2004
		{
			fRepairH = InstantRepairRATE  + iBonus - hpp; // boal 23.01.2004
		...
		if(spp < InstantRepairRATE + iBonus && nMaterialS>0) // boal 23.01.2004
		{
			fRepairS = InstantRepairRATE + iBonus -spp; // boal 23.01.2004
...
float ProcessSailRepair(ref chref, float repPercent)
{
	...
	for(i=0; i<rq; i++)
	{
		...
		for(j=0; j<gq; j++)
		{
			...
			//HardCoffee определяем парус, который невозможно поставить
			//я хз для чего нужен предыдущий mastFall он по ходу не работает
			if (CheckAttribute(arSail,"brokenMast")) continue;
	...
	if (CheckAttribute(chref, "ship.SP")) chref.ship.sp = stf(chref.ship.sp) + repPercent;
	//chref.ship.sp = CalculateShipSP(chref); //HardCoffee фикс отображения сотояния парусов (CalculateShipSP()) теперь возвращает значение из аттрибута chref.ship.sp
...
{
	//HardCoffee Дамаг по парусам уже посчитан в ProcessRandomSailDmg() и вычтен с учётом сломанных мачт
	if (CheckAttribute(chref, "ship.SP")) return stf(chref.ship.SP);
...
//boal <--
	...
	// evganat - энциклопедия
	int iBonus = GetParamPageBonus("InstantRepair");
	bool bYesHPRepair = chrShipHP < InstantRepairRATE + iBonus;// boal 23.01.2004
	bool bYesSPRepair = chrShipSP < InstantRepairRATE + iBonus; // boal 23.01.2004
