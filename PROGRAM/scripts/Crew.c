...
int GetSalaryForShip(ref chref)
{
	...
	// экипаж
	nPaymentQ += GetSalaryForCrew(chref, GetCrewQuantity(chref));
...
void AddPartyCrewMorale(ref chr, int add)
{
	ref sld;
	int idx;
	...
	for(int i = 0; i < COMPANION_MAX; i++)
	{
		idx = GetCompanionIndex(chr,i);
		if(idx >= 0 && GetRemovable(&Characters[idx]))
		AddCrewMorale(&Characters[idx], add);
	}
}
...
int GetCharacterRaiseCrewMoraleMoney(ref chr)
{
	...
	float fExp = (GetCrewExp(chr, "Sailors") + GetCrewExp(chr, "Cannoners") + GetCrewExp(chr, "Soldiers")) / 100.0; // средний коэф опыта 0..3
...
int GetCrewPriceForTavern(string sColony)
{
	if (GetGlobalTutor())
	{
	    return sti(pchar.questTemp.BloodLine_GTutor.hireCrewMoney);
	}

	int iColony = FindColony(sColony);
	ref rTown = &colonies[iColony];
	
	float fExp = (GetCrewExp(rTown, "Sailors") + GetCrewExp(rTown, "Cannoners") + GetCrewExp(rTown, "Soldiers")) / 100.0; // средний коэф опыта 0..3
...
void Partition_SetValue(string state) // state = "before" || "after" - для сравнения было-стало
{
	...
	for(i=0; i < iMax; i++)
	{
		...
		{
			chref = GetCharacter(cn);
			if (GetRemovable(chref) || CheckAttribute(chref, "Capellan")) // считаем только своих
			{
...
void ChangeMoraleDueToDebt(bool bNewMonth) //HardCoffee ref снижение лояльности и морали от долга
{
	if (bIsRepairingProcess) return;
	if (!CheckAttribute(pchar, "CrewPayment")) return;
	int i, cn;
	ref rChr;
	float fMoraleKoef;

	if (bNewMonth) fMoraleKoef = 37.0; //Больше урона по морали при отказе платить зп в начале месяца
	else fMoraleKoef = 12.0;  //Небольной урон по морали каждый день
	fMoraleKoef = fMoraleKoef - GetSummonSkillFromNameToOld(pchar, SKILL_LEADERSHIP);

	if (CheckOfficersPerk(pchar, "IronWill")) fMoraleKoef /= 2;

	int iRnd = makeint(fMoraleKoef + 10);
	...
	for (i = 0; i < COMPANION_MAX; i++)
	{
		...
		if (GetCrewQuantity(rChr) < 1) continue;
		AddCrewMorale(rChr, -makeint(fMoraleKoef));

		if (rand(iRnd) > 8 && CheckAttribute(rChr, "loyality") && !CheckAttribute(rChr, "OfficerWantToGo.DontGo"))
		{
			ChangeCharacterLoyality(rChr, -1);
		}
	}

	for (i = 0; i < GetOfficerPassengerQuantity(pchar); i++)
	{
		cn = GetPassenger(pchar, i);
		if (cn < 0) continue;
