...
void SelectColony(int iColony)
{
	...
	string sColony	= rColony.id;

	int iColor;
	int iDistance = CalculateDistancePapermap(iColony);
	...
	sText = XI_ConvertString("ColonyLevel" + rColony.colonylevel);
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);
	...
	sText = rColony.wheatfield;
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);

	sText = rColony.fishingpier;
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);
	...
	sText = rColony.shipyard;
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);

	sText = rColony.academy;
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);
	...
	sText = rColony.church;
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);

	sText = rColony.storehouse;
	sText = sText + " ";
	SendMessage(&GameInterface,"lsle",MSG_INTERFACE_MSG_TO_NODE,"COLONY_INFO", 0,&sText);

	if(CheckAttribute(rColony, "info.day"))
	{
		sText = XI_ConvertString("LastInfoFromColony");
		sText = sText + rColony.info.day + " " + XI_ConvertString("target_month_" + rColony.info.month) + " " + rColony.info.year + ".";
	}
	SetFormatedText("COLONY_TIME_INFO", sText);

	sText = XI_ConvertString("ColonyDistance") + " "  + FindDaysString(iDistance);
...
// TODO del -->
int CalculateDistancePapermap(int iColonyNumber)
{
