...
void ClearAllFire() // boal new
{
	...
	{
		// delete particles from ship/etc
		if (Ships[i] >= 0) SendMessage(&Characters[Ships[i]], "l", MSG_SHIP_SAFE_DELETE);
	}
}
...
void Sea_ClearCheckFlag() 
{
    if (bSeaActive)
    {
		...
		{
			if (Ships[i] >= 0 && !CheckAttribute(&Characters[Ships[i]], "DontCheckFlag"))
			{
...
void Ship_FireAction()
{
	...
	//--> Rosarak. Квестовое
	if(CheckAttribute(rCharacter, "QuestFireAction"))
	{
		DeleteAttribute(rCharacter, "QuestFireAction"); //Срабатывает 1 раз
		
		switch(rCharacter.id)
		{
			case "JusticeOnSale_ShipPirate":
				if(!CheckAttribute(Pchar, "GenQuest.JusticeOnSale.MayorQuest"))
				{
				...
				else
				{
					...
					pchar.quest.JusticeOnSale_ExitFromSea.win_condition.l1 = "ExitFromSea";
					pchar.quest.JusticeOnSale_ExitFromSea.function = "JusticeOnSale_ExitFromSea"; 	
				}
			break;
		}
	}
	...
	for (i=0; i<iNumShips; i++)
	{
		...
		//вообще неясно, для чего это, если ниже всё одно атака групп
...
float Ship_MastDamage()
{
	...
	switch (iDamageType)
	{
		case SHIP_MAST_TOUCH_ISLAND:
			if(CheckAttribute(rCharacter, "AlwaysSandbankManeuver")) return 0.0;
...
// fix "no answer from SHIP_HULL_DAMAGE evt"
...
//	Ship_SetSailState(sti(rCharacter.index), 0.5);	
}
...
void Ship_Add2Sea(int iCharacterIndex, bool bFromCoast, string sFantomType, bool isOffline)
{
	...
	DeleteAttribute(rCharacter, "ship.cannons.chance_for_explode"); //TODO у нас не юзается
    ...
    rCharacter.TmpSkillRecall = "0"; //boal 25.05.04 оптимизация расчёта, когда == 0 считаем скилы
...
void Ship_BortReloadEvent()
{
	...
	if (sti(aCharacter.Tmp.BortsReloaded.Event))
	{
		...
		if (bLeft)  { PlayVoice("BortReload_cannonl"); }
		if (bRight) { PlayVoice("BortReload_cannonr"); }
	}
...
void Ship_CheckSituation()
{
	...
	// boal -->
	if (CheckAttribute(rCharacter, "ShipCannonChargeType")) // офам приказ, чем палить всё время
	{
	...
	else
	{
	    ...
	    {
	        int iCannonType = sti(rCharacter.Ship.Cannons.Type);
	        //если нет орудий то и не следим за дистанцией...
	        if (iCannonType != CANNON_TYPE_NONECANNON)
	        {
		...
		switch (sti(rCharacter.SeaAI.Task))
		{
			...
			case AITASK_BRANDER:
				...
				if (fBranderDistance > fDistance)
				{
					...
					Log_Info(StringFromKey("InfoMessages_111", XI_ConvertString(rBaseShip.BaseName), rCharacter.Ship.Name, shipSex));
					return;
				}
	        ...
	        else
	        {
				...
				if (!CheckAttribute(rCharacter, "SinkTenPercent")) // это не ЧЖ и не идущие на абордаж пираты
	            {
                    ...
                    if (sti(rCharacter.Ship.LastBallCharacter) != -1 && !CheckAttribute(rCharacter, "Ship_SetTaskAbordage") && !CheckAttribute(rCharacter, "ShipTaskLock")) // нет приказа на абордаж
                    {
				        ...
				        {
							if ((SailsPercent > 50) && (HPPercent > 30) && CheckAttribute(rCharacter, "SeaAI.Task") && sti(rCharacter.SeaAI.Task) != AITASK_RUNAWAY) // не убегает уже
					        {
                        ...
                        if (!CheckAttribute(rCharacter, "ShipTaskLock"))
						{
				            if (!CheckAttribute(rCharacter, "SeaAI.Task.Target") || sti(rCharacter.SeaAI.Task.Target) == -1)
					        {
								if (CheckAttribute(rCharacter, "SeaAI.Task") && sti(rCharacter.SeaAI.Task) != AITASK_RUNAWAY)
								{
									...
									//в таком виде оно работать адекватно не будет, пусть знающие люди правят это говно
									//сделал как оно работало, только без скобок которые руинят парсер плагина
										Ship_SetTaskAttack(SECONDARY_TASK, sti(rCharacter.index), GetMainCharacterIndex());
										Ship_SetTaskRunaway(SECONDARY_TASK, sti(rCharacter.index), GetMainCharacterIndex());
									/*	в таком виде оно работать адекватно не будет, пусть знающие люди правят это говно
									{
		        ...
		        else
		        {
					...
					{
					/*
					    float fEnemyBoxSizeX = stf(rCharacter.Ship.BoxSize.x) / 2.0;
					    float fOurBoxSizeX = stf(pchar.Ship.BoxSize.x) / 2.0;
                        float fAbordageDistance = Ship_GetDistance2D(GetMainCharacter(), rCharacter) - fEnemyBoxSizeX - fOurBoxSizeX;
                        float fOurMaxAbordageDistance = Ship_CalcMaxAbordageDistance(stf(pchar.TmpSkill.Grappling), Ship_CalcGrapplingPerk(pchar));

                        if (fAbordageDistance < fOurMaxAbordageDistance)
						{
//							int SmallPearlQty = rand(250) + 100;
//							int BigPearlQty = rand(150) + 50;
//							AddItemLog(pchar, "jewelry12,jewelry11", "" + SmallPearlQty + "," + BigPearlQty, StringFromKey("InfoMessages_123", rCharacter.ship.name, SmallPearlQty, BigPearlQty), "Important_item");

                            int pinctadaQty = GetCargoGoods(rCharacter, GOOD_PINCTADA);
                            SetCharacterGoods(pchar, GOOD_PINCTADA, GetCargoGoods(pchar, GOOD_PINCTADA) + pinctadaQty);
                            SetCharacterGoods(rCharacter, GOOD_PINCTADA, 0);
                            LogSound(StringFromKey("InfoMessages_123", rCharacter.ship.name, pinctadaQty), "Important_item");
...
//							pchar.questTemp.Sharp.PearlAreal.BigPearlQty = sti(pchar.questTemp.Sharp.PearlAreal.BigPearlQty) + BigPearlQty;
							pchar.questTemp.Sharp.PearlAreal.PinctadaQty = sti(pchar.questTemp.Sharp.PearlAreal.PinctadaQty) + pinctadaQty;
							...
							SetCharacterRelationBoth(sti(rCharacter.index), GetMainCharacterIndex(), RELATION_NEUTRAL);
                            RefreshBattleInterface();
						}
                    */
					}
	    ...
	    else
	    {
	        ...
	        // boal 030804 узнал на днях, что можно это всё делать оин раз и оно ПОМНИТ!!!  но тогда нужно ИД проверять, тк перекрываем выше
	        if (CheckAttribute(rCharacter, "AlwaysEnemy"))
	        {
	            SetCharacterRelationBoth(sti(rCharacter.index), GetMainCharacterIndex(), RELATION_ENEMY);
	            Ship_SetTaskAttack(SECONDARY_TASK, sti(rCharacter.index), GetMainCharacterIndex());
	        ...
	        //UpdateRelations();  // TODO это тонкое место, это тормоз, но рефрешить нужно
	    }
	    // AI компа <--
    }
}
...
void Ship_CheckFlagEnemy(ref rCharacter)
{
    ...
    {
        // TODO ch.CheckLicence    = true; // Проверять наличие торговой лицензии при распознании,нужно фортам и сторожевикам
    ...
    {  // смотрим флаг у ГГ и узнаем в нём...
        ...
        int iRel = GetCharacterReputation_WithNation(pchar, sti(rCharacter.nation));
        if (iRel <= 20) iRel = 0;
		if ((rand(100) + rand(20) + rand(rep) - iRel - GetCharacterSPECIAL(pchar, "Luck")) > (fSneak * 10 * GetPcharSquadronAverageClass(false) * (6-iCompan)))
		{
			/*mChar.nation = iNationToChange;  // TODO ролик флага сделать
			Ship_FlagRefresh(PChar); //флаг на лету
	        SetNationToOfficers(iNationToChange);
	        SetNationRelation2MainCharacter(sti(rCharacter.nation), RELATION_ENEMY);
	        */
//			Log_Info(StringFromKey("InfoMessages_124", NationNamePeople(sti(rCharacter.nation))));
			Notification(StringFromKey("InfoMessages_124", NationNamePeople(sti(rCharacter.nation))), "SneakFail");
			SetCharacterRelationBoth(sti(rCharacter.index), GetMainCharacterIndex(), RELATION_ENEMY);
			DoQuestCheckDelay(NationShortName(iNationToChange) + "_flag_rise", 0.1); // применение нац отношений флага
			if (!CheckAttribute(pchar,"questTemp.stels.sea") || sti(pchar.questTemp.stels.sea) != GetDataDay())
			{
		...
		{ // не узнал
		    Notification(StringFromKey("InfoMessages_227", NationNamePeople(sti(rCharacter.nation))), "Sneak");
			if (!CheckAttribute(pchar,"questTemp.stels.sea") || sti(pchar.questTemp.stels.sea) != GetDataDay())
			{
...
int Ship_FindOtherBallType(ref rCharacter, float fMinEnemyDistance, bool bBalls, bool bBombs, bool bGrapes, bool bKnippels)
{
	...
	if (checkAttribute(rCharacter, "SeaAI.task.target") && rCharacter.SeaAI.task.target != "") //fix
	{
		...
		if ( (fMinEnemyDistance <= (range * 0.6)) && bKnippels) 
		{
		...
		{
			if (sti(characters[enemy].ship.crew.quantity) > sti(rBaseShip.MinCrew) && GetHullPercent(characters[enemy]) < 50)	// проаерка на прочность корпуса, чтоб по целому кораблю картечью не лупил ZhilyaevDm
			{
				iType = GOOD_GRAPES;  
			}
			fRangeModifier = 0.3;
...
{         // че-то распук метод "неподецки" - переделал 29.07.06 boal
	...
	if (CheckOfficersPerk(rOurCharacter, "Doctor3"))
	{
		...
		if (!ENCYCLOPEDIA_DISABLED)
		{
			if (IsMainCharacter(rOurCharacter) || IsCompanion(rOurCharacter))
				iBonus = GetParamPageBonus("Doctor3");
		}
		fMultiply = fMultiply * (0.8 - 0.01 * iBonus);
	}
	else if (CheckOfficersPerk(rOurCharacter, "Doctor2"))
	{
		fMultiply = fMultiply * 0.85;
	}
	else if (CheckOfficersPerk(rOurCharacter, "Doctor1"))
	{
	...
	if (fNewCrewQuantity >= f5Percent && IsCompanion(rOurCharacter))
	{
       ...
       ChangeCrewExp(rOurCharacter, "Soldiers",  (fCrewHP / stf(rBaseShip.OptCrew))); // TODO может быть много
    }
    // boal <--
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    if (iKillerCharacterIndex >= 0)
	{
		if (sti(Characters[iKillerCharacterIndex].TmpPerks.CannonProfessional))
		{
			// evganat - энциклопедия
			if (!ENCYCLOPEDIA_DISABLED)
			{
				if (IsMainCharacter(&Characters[iKillerCharacterIndex]) || IsCompanion(&Characters[iKillerCharacterIndex]))
					iBonus = GetParamPageBonus("CannonProfessional_damage");
			}
			fPlus = 0.3 + 0.01 * iBonus;
		}
		else if (sti(Characters[iKillerCharacterIndex].TmpPerks.HullDamageUp)) fPlus = 0.15;
		//fAccuracy = stf(characters[iKillerCharacterIndex].TmpSkill.Accuracy);
	}

	if (sti(rOurCharacter.TmpPerks.ShipDefenseProfessional))	
	{
		// evganat - энциклопедия
		if (!ENCYCLOPEDIA_DISABLED)
		{
		...
		fMinus = 0.35 + 0.01 * iBonus;
	}
	else if (sti(rOurCharacter.TmpPerks.AdvancedBattleState)) fMinus = 0.25;
	else if (sti(rOurCharacter.TmpPerks.BasicBattleState)) fMinus = 0.15;
	/*if (bNoDamage2Us)
	{
...
void ShipDead(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    {
        Log_TestInfo("Error: ShipDead Нет корабля у iDeadCharacterIndex = "+iDeadCharacterIndex);
        return;
    }
	...
	if (iDeadCharacterIndex != nMainCharacterIndex)
	{
		...
		{
            sSunkString = sSunkShipType + " '" + rDead.Ship.Name + "' " + GetShipSexWord(rBaseShip.BaseName, StringFromKey("SexPhrase_29"), StringFromKey("SexPhrase_30")) + " " + GetFullName(rKillerCharacter);
        }
        else
        {
            if (sKillShipName == "")
            {
            	sSunkString = sSunkShipType + " '" + rDead.Ship.Name + "' " + GetShipSexWord(rBaseShip.BaseName, StringFromKey("SexPhrase_29"), StringFromKey("SexPhrase_30"));
            }
            else
            {
				sSunkString = sSunkShipType + " '" + rDead.Ship.Name + "' " +GetShipSexWord(rBaseShip.BaseName, StringFromKey("SexPhrase_29"), StringFromKey("SexPhrase_30")) + " " + sKillShipType + " '" + sKillShipName + "'";
			}
        }
//		Log_SetStringToLog(sSunkString);
 	...
 	if (rand(10) >= 6) // глупое условие... это всё классы  && sti(rBaseShip.Class) <= 6
	{ 
...
*/
	...
	if(rDead.id == "GhostCapt" && iKillerCharacterIndex == nMainCharacterIndex && !CheckAttribute(pchar, "GenQuest.GhostShip.LastBattle") && !CheckAttribute(pchar, "GenQuest.GhostShip.Prize"))
	{
		...
		Achievment_Set(ACH_Uzrevshiy_Legendu);
	}
	if (iDeadCharacterIndex == nMainCharacterIndex)
	{ 
		...
		FadeOutMusic(2);
		// утонул в ГПК
		if (CheckAttribute(pchar, "questTemp.LSC.drownedman"))
		{
		    DoQuestFunctionDelay("ReloadToLSC", 6.0);
		}
		else
		{
            // убил ЛГ
	...
	else
	{
        if (bDeadCompanion && CheckOfficersPerk(rDead, "ShipEscape") && GetRemovable(rDead)) // выживаем
        {
            ...
            Log_Info(StringFromKey("InfoMessages_125", GetFullName(rDead)));
        }
	}
	// спасем офицеров boal 07/02/05
...
void ShipTaken(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    if (iKillerCharacterIndex != -1 && !bDeadCompanion)
	{
        ...
        // boal statistic info 17.12.2003 <--
        // AlexBlade для ачивок -->
        Achievement_AddStats_AbordShip();
        // <--

        ChangeCrewExp(rKillerCharacter, "Sailors", 0.5);
		// в лаи_бординг ChangeCrewExp(rKillerCharacter, "Soldiers", 5); // TODO перенести на форму захвата
	}
...
void ShipTakenFree(int iDeadCharacterIndex, int iKillStatus, int iKillerCharacterIndex)
{
    ...
    if (iKillerCharacterIndex != -1)
	{
		...
		Statistic_AddValue(rKillerCharacter, "AbordShipFree_" + rBaseShip.Class, 1);  // эту статистику пока нигде не видно TODO
...
void Ship_HullHitEvent()
{
	...
	if (sti(rBallCharacter.TmpPerks.CannonProfessional))
	{
		// evganat - энциклопедия
		if (!ENCYCLOPEDIA_DISABLED)
		{
			if (IsMainCharacter(rBallCharacter) || IsCompanion(rBallCharacter))
				iBonus = GetParamPageBonus("CannonProfessional_critical");
		}
		if (rand(99) < 10 + iBonus) bSeriousBoom = true; // +10%
	}
	else if (sti(rBallCharacter.TmpPerks.CriticalShoot) && rand(19) == 10) bSeriousBoom = true; // +5%
    ...
    if (sti(rOurCharacter.TmpPerks.ShipDefenseProfessional))
	{
		...
		{
			if (IsMainCharacter(rBallCharacter) || IsCompanion(rBallCharacter))
				iBonus = GetParamPageBonus("ShipDefenseProfessional_critical");
		}
		if (rand(999) < 700 + 10 * iBonus) bSeriousBoom = false; // no seriouse boom
	}

	// --->>>	ZhilyaevDm
				// Модификатор урона по команде корабля от процента прочности корпуса
				// Чем сильнее повреждения, тем больше урон. Если корпус полностью целый, урон минимален.
/*	aref	arCharShip; makearef(arCharShip, rOurCharacter.Ship);
	ref		rShip = GetRealShip(sti(arCharShip.Type));

	float fCrewDamageMultiply = 0.0;
	float fCrewHullProtection = (stf(arCharShip.HP) / stf(rShip.HP));
...
//			Log_Info(StringFromKey("InfoMessages_128", UpperFirst(XI_ConvertString("musicmod_s")), rOurCharacter.Ship.Name));
	...
	// boal 290704 раз в минуту проверяем обиду на гл героя, если жухлит через жёлтый прицел -->
...
{
	ref rBaseShip = GetRealShip(sti(rFantom.Ship.Type));
...
float Ship_CalcGrapplingPerk(ref rCharacter)
{
	...
	// evganat - энциклопедия
	int iBonus = 0;
	if (!ENCYCLOPEDIA_DISABLED && CheckOfficersPerk(rCharacter, "GrapplingProfessional"))
	{
		if (IsMainCharacter(rCharacter) || IsCompanion(rCharacter))
			iBonus = GetParamPageBonus("GrapplingProfessional");
	}
	fGrapplingPerk = AIShip_isPerksUse(rCharacter.TmpPerks.GrapplingProfessional, fGrapplingPerk, 1.25 + 0.01 * iBonus);
...
void Ship_CheckShipToFort(aref arShip, aref arFort)
{
	ref rCharacter = GetCharacter(sti(arShip.idx));
	ref rFortCharacter = GetCharacter(sti(arFort.idx));
	if (LAi_IsDead(rCharacter) || LAi_IsDead(rFortCharacter)) { return; }
	int iRelation = sti(arFort.relation);
	float fDistance = stf(arFort.Distance);
	if (iRelation != RELATION_ENEMY)
	{
		return;
	}
...
//Checks ship1
void Ship_CheckShipToShip(aref arShip1, aref arShip2)
{
	ref rCharacter = GetCharacter(sti(arShip1.idx));
	ref rShip2Character = GetCharacter(sti(arShip2.idx));
	if (LAi_IsDead(rCharacter) || LAi_IsDead(rShip2Character)) { return; }

	float fDistance = stf(arShip2.Distance);		// distance between ship1 and ship2
	int iRelation = sti(arShip2.relation);
	
	if (iRelation != RELATION_ENEMY) { return; }
...
void Ship_CheckShipsAroundMainCharacter(aref aShips)
{
	int i, j;
	aref aForts, aShipsAround;
	int iShipsNum = GetAttributesNum(aShips);
	
	for (i=0; i<iShipsNum; i++)
	{
		aref aShip = GetAttributeN(aShips, i);
		ref rCharacter = GetCharacter(sti(aShip.idx));
		if (iStormLockSeconds)
		{
			rCharacter.Ship.POS.Mode = SHIP_STORM;
			continue;
		}
		if (!CheckAttribute(rCharacter, "SeaAI.Update"))
		{
			continue;
		}
		rCharacter.Ship.POS.Mode = SHIP_SAIL;
		
		if (CheckAttribute(rCharacter, "SeaAI.Update.Ships"))
		{
			makearef(aShipsAround, rCharacter.SeaAI.Update.Ships);
			int iShipsAroundNum =  GetAttributesNum(aShipsAround);
			for (j=0; j<iShipsAroundNum; j++)
			{
				aref aShip1 = GetAttributeN(aShipsAround, j);
				Ship_CheckShipToShip(aShip, aShip1);
			}
		}

		if (CheckAttribute(rCharacter, "SeaAI.Update.Forts"))
		{
			makearef(aForts, rCharacter.SeaAI.Update.Forts);
			int iFortsNum = GetAttributesNum(aForts);
			for (j=0; j<iFortsNum; j++)
			{
				aref aFort = GetAttributeN(aForts, j);
				Ship_CheckShipToFort(aShip, aFort);
			}
		}
	}
}
...
void Ship_CheckMainCharacter()
{
	...
	// tutorials check
	if (sti(rCharacter.Ship.POS.Mode) == SHIP_WAR)
	{
		PostEvent("Event_CheckSeaBattleTutorials", 2000);
	}
	...
	for (i=0; i<iFortsNum; i++)
	{
		...
		switch (iFortMode)
		{
			...
			case FORT_DEAD:
				...
				if (GetGlobalTutor()) { continue; } // fix для Блада
	...
	// AlexBlade > Установим People on ship (POS) для кораблей в округе
	Ship_CheckShipsAroundMainCharacter(aShips);
		...
		if (Island_isReloadEnableGlobal(sPlayerLocation))
		{
			...
			for (int j=0; j<iNumLocators; j++)
			{
				...
				if (sqrt(sqr(x1 - x) + sqr(z1 - z)) < r)
				{
					...
					pchar.NearestLandLocatorInRadius = rIsland.reload.(sLocatorName).go;	// evganat - пасха
					//HardCoffee отображение места причаливания в интерфейсе
					BattleInterface.SeaActiveActionName = "nothing";
					BattleInterface.textinfo.Location.text = LanguageConvertString(iLocLablesFile, rIsland.reload.(sLocatorName).label);
					break;
				}
				else
				{
					pchar.NearestLandLocatorInRadius = ""; // evganat - пасха
					if (worldMap.island != "" && worldMap.island != "LostShipsCity") //отобразить название акватории
					{
						BattleInterface.textinfo.Location.text = LanguageConvertString(iLocLablesFile, worldMap.island);
					    BattleInterface.textinfo.Location.text = BattleInterface.textinfo.Location.text + " - " + LanguageConvertString(iLocLablesFile, "Sea");
					}
				}
			}
		}
	}
	if (BattleInterface.textinfo.Location.text == "")
...
void Ship_EndStorm()
{
    ...
    bStorm = false;
    if (bTornado) WhrDeleteTornadoEnvironment();
    bTornado = false;
    WeatherParams.Storm = false;
    WeatherParams.Rain = true;

    //начинается просто дождь
    iCurWeatherNum = FindWeatherByHour(makeint(GetTime()));
    iBlendWeatherNum = FindBlendWeather(iCurWeatherNum);
    WeatherParams.Rain.ThisDay 		= true;
    WeatherParams.Rain.StartTime = GetTime() - 1.0; 	// время старта с минутами
    WeatherParams.Rain.Duration  = 90 + rand(90);
    ...
    if (sti(WeatherParams.Rain.Type))	Whr_SetRainExt1(iCurWeatherNum, iBlendWeatherNum, true);
    else								Whr_SetRainExt2(iCurWeatherNum, iBlendWeatherNum, true);

    fWeatherSpeed = 20.0; //читирим с силой ветра, так как нам не нужен рандом, а волны нам нужны
...
void Ship_UpdateParameters()
{
	...
	arCharShip.MaxSpeedZ = Sea_ApplyMaxSpeedZ(rCharacter, arCharShip, fWindDotShip); // учет парусов в др месте
	...
	// ===============
	// boal далее я всё закоментил, тк мешало работать коду от скорости, да и вообще сомнительно это для маневра.
	...
	*/
	...
	{
		float fSinkDamage = AIShip_isPerksUse(arTmpPerks.Caulking, 1.0, 0.0) * (fBaseShipHP / 100.0) * 0.08;
        if (CheckAttribute(rCharacter, "SinkTenPercent") && !sti(rCharacter.SinkTenPercent)) // это спец атрибут по убиванию этой функции
			fSinkDamage = 0.0;
		...
		if (fSinkDamage > 0.0)
		{
			...
			/*if(!CheckAttribute(rCharacter, "ship.flow")) // TODO
			{
	...
	// evganat - энциклопедия
	int iBonus = 0;
	if (!ENCYCLOPEDIA_DISABLED && CheckOfficersPerk(rCharacter, "StormProfessional"))
	{
		if (IsMainCharacter(rCharacter) || IsCompanion(rCharacter))
			iBonus = GetParamPageBonus("StormProfessional");
	}
	float fStormProfessional = AIShip_isPerksUse(arTmpPerks.StormProfessional, 1.0, 0.7 - 0.01 * iBonus);
			...
			if (!bSkipTarget)
			{
				...
				if (sti(rCharacter.SeaAI.Task) == AITASK_RUNAWAY)
				{
					...
					*/
					...
					Ship_SetSailState(sti(rCharacter.index), 1);  // TODO нужно проверять, может довольно одного раза или в ядре будет дурить
							...
							if(LAi_IsDead(rCharacter) == false && LAi_IsDead(rTargetCharacter) == false)
							{
								if (sti(rTargetCharacter.index) == nMainCharacterIndex)
								{
									...
									if (fAbordageDistance < fCharacterDistance && fAng < fCharacterAngle && fRelativeSpeed < fCharacterSpeed)
									{
										...
										else  */ // код К3 пошёл лесом, тк иначе случаи захвата очень редки
										//if (sti(rTargetCharacter.index) == nMainCharacterIndex)
										//{
										if (fRatio > 1.2)
										{
...
void Ship_UpdatePerks()
{
	...
	aTmpPerks.Caulking					= CheckOfficersPerk(rCharacter, "Caulking");
...
void SetTextureForShip(aref refBaseShip, aref rCharacter)
{
	if (!CheckAttribute(refBaseShip, "name"))
	{
		...
		if (MOD_BETTATESTMODE == "On")
			Log_Info("SetTextureForShip(): ship " + refBaseShip.basetype + " have no name! NPCid = "+ rCharacter.id);
		return;
	}

	string sPathShips = "ships/";
	string sPathNames = sPathShips + refBaseShip.name + "/";
	string sPathHulls = sPathNames + "hull" + refBaseShip.ship.upgrades.hull + "/";

	SetTexturePath(0, sPathShips);
	SetTexturePath(1, sPathNames);
	SetTexturePath(2, sPathHulls);
}
...
void DropGoodsToSea()
{
	...
	for (int i=GOOD_MEDICAMENT; i<GOOD_CULVERINE_32; i++)
	{
		...
		if(CheckAttribute(rCharacter, "Ship.Cargo.Goods."+sGood))
		{
			...
			//RecalculateCargoLoad(rCharacter);
            rCharacter.Tmp.SpeedRecall = 0; // чтоб пересчитались скорость и манёвр
...
// evganat - выделил сокращение урона по команде в отдельную функцию
float GetCrewDamageReduction(ref chr)
{
	int iClass = GetCharacterShipClass(chr);
	float curHP = GetHullPercent(chr);
	float kDmg = 1.0;
	float lLim = 0.0;
	float rLim = 100.0;
	float kLim = 1.0;
	switch(iClass)
	{
		case 6:	lLim = 70.0;	rLim = 90.0;	kLim = 0.85;	break;
		case 5:	lLim = 65.0;	rLim = 85.0;	kLim = 0.75;	break;
		case 4:	lLim = 60.0;	rLim = 85.0;	kLim = 0.65;	break;
		case 3:	lLim = 55.0;	rLim = 80.0;	kLim = 0.55;	break;
		case 2:	lLim = 45.0;	rLim = 80.0;	kLim = 0.45;	break;
		case 1:	lLim = 35.0;	rLim = 75.0;	kLim = 0.35;	break;
	}
	kDmg = Bring2Range(1.0, kLim, lLim, rLim, curHP);
	return kDmg;
}

void ProcessQuestShipSightLog(ref chr)
{
	bool canSpeakShip = StrStartsWith(chr.id, "SeekCap_") || chr.id == "QuetionsVillemstad_MuzhTorgovets" ||
		StrStartsWith(chr.id, "PortmansCap_") || StrStartsWith(chr.id, "SeekCitizCap_") ||
		and(chr.id == "Sharp", pchar.questTemp.Sharp == "seekSharp" || pchar.questTemp.Sharp == "toSharp_going");

	bool onlyFightShip = chr.id == "MushketCap";
    if (canSpeakShip || onlyFightShip)
    {
        LogSound_WithNotify(StringFromKey("InfoMessages_226", XI_ConvertString(GetShipTypeName(chr)), chr.Ship.name), "", "ShipSight");
    }
	if (canSpeakShip)
    {
		PostEvent("ProcessQuestShipCanSpeak", 2500, "i", chr);
    }
}

#event_handler("ProcessQuestShipCanSpeak", "ProcessQuestShipCanSpeak");
void ProcessQuestShipCanSpeak()
{
