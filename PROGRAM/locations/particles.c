...
*/
	...
	//Rosarak. Нормальная реализация костров в джунглях
	if(CheckAttribute(Location, "bonfire"))
	{
		ref rItm = ItemsFromID("fire");
		if(sti(Location.bonfire))
		{
			rItm.shown = true;
	...
	if (CheckAttribute(Location, "Orange_mines_fireLSC"))
	{
	...
	if (CheckAttribute(Location, "FortOrange_ExitTown_fireLSC"))
	{
...
void CreateHeaters(aref Location)
{
	...
	if(CheckAttribute(Location, "locators.coldfires_p"))
	{
		bool isColdfiresEnable = true;
		if(CheckAttribute(location, "coldfires_night_only"))
		{
			if(CheckAttribute(&Weathers[iCurWeatherNum], "Lights"))
			{
				if(sti(Weathers[iCurWeatherNum].Lights) == 0)
				{
					isColdfiresEnable = false;
				}
			}
		}
		if(CheckAttribute(location, "coldfires_disable"))
		{
			if(sti(location.coldfires_disable) == 1)
			{
				isColdfiresEnable = false;
			}
		}

		if(isColdfiresEnable == true)
		{
			makearef(locator_group, location.locators.coldfires_p);
			num = GetAttributesNum(locator_group);
			for(n = 0; n < num; n++)
			{
				locator = GetAttributeN(locator_group, n);
				CreateParticleSystem("coldFire",stf(locator.x),stf(locator.y),stf(locator.z),-1.57,0,0,0);
			}
		}
	}
}
...
//Jason: универсальный создатель партиклов в локации ручным способом. Создает партикл в указанном локаторе или рядом с ним по оси Y и добавляет звук.
//up - на сколько над локатором должен быть поднят/опущен партикл.
//ax и az - корректировщики для соотв. осей при ориентировании направленных партиклов.
//Звуки брать из алиаса. Список некоторых партикловых схем:
//  blood/blood_big - брызги крови
//  stream - ручеек
//  Splash - водный всплеск
//  item - фиолетовая крутящаяся звезда
//  Firering_teho - мигающее желтое мистическое кольцо
//  Ship_cannon_fire - выстрел из пушки
//  blast_inv/Bombard - вспышка+дым
//  blast/blast_dirt - взрыв с летающими какашками
//  GunFire - выстрел из пистоля
//  fountain - фонтан
//  bubbles - пузыри
//  fishblue - косяк рыбок
//  shipfire - большой огонь
//  torch - факел
//  shadowstar - теневая звезда
//  large_smoke - большой дым
//  smoke - дым
//  ShipExplode - взрыв
...
bool CreateLocationParticlesXZ(string sParticle, string sGroup, string sLocator, float up, float ax, float az, string sSound)
{
	...
	CreateParticleSystemX(sParticle, x+ax, y+up, z+az, x+ax, y+up, z+az, 10);
	if (sSound != "")
			Play3DSound(sSound, x, y, z);
	return true;
}

//то же самое, только звук зациклен
bool CreateLocationParticlesLoopedSound(string sParticle, string sGroup, string sLocator, float up, float ax, float az, string sSound)
{
	ref rLoc = &locations[FindLocation(PChar.location)];
	...
	CreateParticleSystemX(sParticle, x+ax, y+up, z+az, x+ax, y+up, z+az, 10);
	if (sSound != "")
			Play3DSoundComplex(sSound, x, y, z, true, false);
	return true;
}