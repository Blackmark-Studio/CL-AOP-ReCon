...
bool CheckMapEnterConditions() {
		...
		if (cn >= 0) {
            ...
            if (GetCargoLoad(chref) > GetCargoMaxSpace(chref)) {
				if (pchar.DirSailFail != "overload") { // чтобы не было спама сообщениями каждый час
					Log_SetStringToLog(StringFromKey("InfoMessages_85", XI_ConvertString("Ship"), chref.Ship.Name));
	...
	if (bLog) {
		Log_Info(StringFromKey("InfoMessages_88"));
	...
	if(bCheckEnc) 
	{
        ...
        if(isStorm) 
		{
            ...
            else 
			{
				...
				Whr_UpdateWeather();
                Log_SetStringToLog(StringFromKey("InfoMessages_89"));
            }
        }
	}	
}
