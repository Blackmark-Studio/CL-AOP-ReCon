...
void LocAi_Init(ref loc)
{
	...
	SetEventHandler("Character_ChangeFightMode", "LAi_CharacterChangeFightMode", 0);
	LAi_IsInitedAI = true;
	...
	if(!actLoadFlag)
	{
		...
		{
			//DelRelations(&Characters[LOC_FANTOM_CHARACTERS + i]);
			//DeleteNPCInRumourGroups(&Characters[LOC_FANTOM_CHARACTERS + i]);
	...
	if(!isBoarding)
	{
		...
		if ( !CheckAttribute(loc, "DisableOfficers") ) //eddy. офицерский флаг
		{
			for(i = 1; i < 4; i++)
			{
				...
				if(Characters[idx].id == "Douglas" && loc.id == "PortSpein_houseH3")
				{
					// Хемфри Дуглас хозяйничает дома
					SetCharacterToMushketer(&Characters[idx], "");
					LAi_SetOwnerTypeNoGroup(&Characters[idx]);
					Characters[idx].DontChangeGun = true;
					Characters[idx].location = "PortSpein_houseH3";
					Characters[idx].location.group = "goto";
					Characters[idx].location.locator = "goto3";
					...
					pchar.quest.Duglas_Officer.win_condition = "Duglas_Officer";
				}
			}
		}
		else
...
void LocAi_PostInit(ref loc)
{
	...
	// KZ > запоминаем абордажников при заходе в локацию
	StoreFighters();
