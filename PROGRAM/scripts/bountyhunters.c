...
void SeaHunterCheck()
{
    ...
    for (j=0; j< MAX_NATIONS; j++)
    {
        ...
        if (GetCharacterReputation_WithNation(pchar, j) <= -10)// Минус это НЗГ
        {
            ...
            for (i = 1; i <= 7; i++)
            {
                ...
                sld.mapEnc.type = "war";
                sld.mapEnc.Name = FindPersonalName("Hunter_mapEnc");
				sld.hunter = "hunter";
                ...
                if (abs(GetCharacterReputation_WithNation(pchar, j)) < (i * 15)) break; // добавим капитана только когда положенно
            }
...
void SetShipHunter(ref Hunter)
{
    ...
    SetCrewQuantityFull(Hunter); // TODO
...
void LandHunterReactionResult(ref loc)  // отработает после входа в локацию, но после квест_реакшна на вход
{
	...
	if (LAi_grp_alarmactive)  return;  // уже ГГ кто-то бьет
	if (CheckAttribute(PChar, "GenQuest.HunterLongPause")) return;
        ...
        if (GetQuestPastDayParam("Land_HunterTimer" + typeHunter) > (7 + rand(7)))
        {
			...
			if (GetCharacterReputation_WithNation(pchar, j) <= -10)// Минус это НЗГ
	        {
	            ...
	            for (i = 1; i <= 8; i++)
	            {
	                ...
	                SetFantomParamHunter(sld); //крутые парни
	                sld.name 	= FindPersonalName("Hunter_name");
	                sld.lastname 	= "";
	                ...
	                if (abs(GetCharacterReputation_WithNation(pchar, j)) < (i * 9)) break; // число от НЗГ
	            }
				...
				if (ok)
	            {
	                pchar.HunterCost = abs(GetCharacterReputation_WithNation(pchar, j));
