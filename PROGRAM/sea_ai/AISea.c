...
void SeaAI_SailToEndFade()
{
	...
	switch (iSailToType)
	{
		case SAIL_TO_LOCATOR:
			...
			//сохраняем координаты куда перемещаемся для покраски воды по этим координатам
			WeatherParams.Whr_ColorizeSeaWater.SAIL_TO_LOCATOR.x = rIslLoc.x;
			WeatherParams.Whr_ColorizeSeaWater.SAIL_TO_LOCATOR.z = rIslLoc.z;
		...
		case SAIL_TO_CHARACTER:
			//сохраняем айди корабля к которому перемещаемся
		    WeatherParams.Whr_ColorizeSeaWater.SAIL_TO_CHARACTER = sSailToString;
...
void SeaAI_SailToCreateFader()
{
	...
	SendMessage(&SailToFader, "ls", FADER_PICTURE0, GetLoadingImage_Sea());
}
...
void SeaAI_SwapShipsAttributes(ref refMyCharacter, ref refEnemyCharacter, bool SetShipNum) // TODO
{
