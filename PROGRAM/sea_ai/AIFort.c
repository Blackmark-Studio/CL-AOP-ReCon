...
void Fort_Login(int iIslandIndex)
{
	...
	for (int i=0; i<iNumReloads; i++)
	{
		...
		if (CheckAttribute(arLocator,"fort"))
		{
			...
			// > Валидация колонии/командира/нации до создания форта
			int iFortCharacter = Fort_FindCharacter(rIsland.id, "reload", arLocator.name);
			int iColony = FindColony(arLocator.colonyname);
			if (iColony < 0)
			{
			...
			{
				case FORT_NORMAL:
				   SetSeaFantomParam(rCharacter, "war"); // boal. генератор!!
				break;
				case FORT_ABORDAGE:
...
int Fort_GetCannonsQuantity(ref rFortCharacter)
{
	...
	Restrictor(&ResultCannons, 0, iMaxCannonsQuantity);
...
float Fort_CannonDamage()
{
	...
	int iFortColony = FindColony(rFortCharacter.City);
	if (iBallCharacterIndex == GetMainCharacterIndex() && iFortColony >= 0 && sti(Colonies[iFortColony].HeroOwn) == true)
	{
	    bImmortal = true;
	    iExp = 200;
...
void Fort_SetAbordageMode(ref rKillerCharacter, ref rFortCharacter)
{
	...
	TEV.FortLandTroops = rFortCharacter.Ship.Name; //Control tips

	if (HasStr(rFortCharacter.id, "Providencia")) // KZ > За вынос форта Провиденсии не давать ничего
		return;
...
{
	if (TestMode()) Trace("Fort_LoadDamagedCannon");
