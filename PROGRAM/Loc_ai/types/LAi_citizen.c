...
{	
	...
	if(chr.chr_ai.tmpl == LAI_TMPL_WALK)
	{
		...
		else
		{
            ...
            {
                if (CheckAttribute(chr, "talker") && !CheckAttribute(chr, "quest.NoTalkerQuestMark"))
                {
                    if(InterfaceStates.EnabledQuestsMarks)
                    {
                        chr.quest.questflag.model = QUESTMARK_GEN;
...
//HardCoffee ref CitizenNotBlade -->
			//--> проверяем не врагов, но дерущихся
			if (LAi_grp_playeralarm == 0 && stf(chr.chr_ai.type.checkFight) < 0.0 && !CheckAttribute(chr, "NoCheckFightMode"))
			{
				num = FindNearCharacters(chr, 3.0, -1.0, 180.0, 0.001, true, true); //Убрать глаза с затылка
				for (i = 0; i < num; i++)
				{
					idx = sti(chrFindNearCharacters[i].index);
					by = &Characters[idx];
					chr.chr_ai.type.checkFight = 1.5;
					if (!LAi_CheckFightMode(by)) continue;

					if (by.id != pchar.id || CheckAttribute(chr, "chr_ai.disableDlg")) //Боимся не только гг
					{
						if (LAi_IsSetBale(chr)) break;
						LAi_tmpl_afraid_SetAfraidCharacter(chr, by, true);
    					return;
					}

					if (LAi_IsUnarmed(by)) continue; //Не агримся, если гг бегает с кулаками

					if (Rand(9) != 0 && !LAi_IsSetBale(chr) && loadedlocation.id != "Santiago_incquisitio")
					{
						LAi_tmpl_afraid_SetAfraidCharacter(chr, by, true);
						return;
					}

					if (LAi_IsOurCitizen(chr)) continue; //Не докапываемся до гг в его городе
					SendMessage(by, "lsl", MSG_CHARACTER_EX_MSG, "SetFightMode", 0); // Убрать оружие без анимации
					if (!LAi_Character_CanDialog(chr, by)) continue; //Пытаемся начать диалог
					//перенёс ноду CitizenNotBlade в отдельный файл, чтобы не было конфликтов с квестодателями
					...
					chr.Dialog.Base_Filename = chr.Dialog.Filename;
					chr.Dialog.Filename = "Common_NotBlade.c";
					chr.Dialog.TempNode = chr.Dialog.CurrentNode;
					if (sti(chr.nation) == PIRATE && LAi_IsSetBale(chr)) chr.Dialog.CurrentNode = "PirateNotBlade";
					else chr.Dialog.CurrentNode = "CitizenNotBlade";
					LAi_tmpl_SetDialog(chr, by, -1.0);
					return;
				}
			}
			else chr.chr_ai.type.checkFight = stf(chr.chr_ai.type.checkFight) - dltTime;
			//<-- проверяем не врагов, но дерущихся.
//HardCoffee ref CitizenNotBlade <--
			if(time <= 0.0)
			{
...
void LAi_type_citizen_Attacked(aref chr, aref by)
{
	...
	//HardCoffee понизить репу если гг начал атаковать гражданских
	if (LAi_grp_playeralarm == 0 && loadedLocation.type == "town" && sti(chr.nation) != PIRATE && by.id == pchar.id)
