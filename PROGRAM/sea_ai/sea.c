...
object	Touch, AISea;
object	SeaFader;
object	Seafoam, BallSplash, SinkEffect, PeopleOnShip, Telescope, SeaOperator;
object	Sharks;
object	SeaLighter;
object  ShipTracks;
...
//Возвращает бухту текущего острова, где бросили якорь, в случае порта - колонию
//Если нужно обязательно вернуть колонию, то ставим true
string Sea_FindNearColony(bool bColony) //ROSARAK, BOAL
{
	aref aLocators;
...
{	
	...
	/* TODO: ЭПИДЕМИИ
	string sColony = Sea_FindNearColony(false);
	...
	if(iColony != -1)
	{
		...
		if(CheckAttribute(&Colonies[iColony], "disease.time"))
		{
			...
			{
				// LaunchDiseaseAlert(DISEASE_ON_COLONY);
				return;
			}
		}
	}*/
...
void Sea_MapLoad()
{
    ...
    for (i=0; i<COMPANION_MAX; i++)
	{
		...
		if( cn>=0 )
		{
            ...
            {
                ok = false;
                Log_SetStringToLog(StringFromKey("InfoMessages_86", XI_ConvertString("Ship"), chref.Ship.Name));
            }
    ...
    if (!ok)
    {
        Log_Info(StringFromKey("InfoMessages_129"));
        PlaySound("knock");
        return;
    }
    // boal 201004 проверка на перегруз и мин команду <--
...
void SeaLogin(ref Login)
{
	...
	if(iPlayerCompanionsQ > 1)
	{
		for (i=1; i<COMPANION_MAX; i++)
		{
			...
			// TODO убрал для пробы, в ПКМ не было и не тупили	Ship_SetTaskNone(PRIMARY_TASK, iCompanionIndex); // сброс для кэпов офов из пленных
			...
			// TODO убрал для пробы, в ПКМ не было и не тупили
	...
	for (i=0; i<iNumGroups; i++)
	{
		...
		{
			sGName = rEncounter.GroupName + rEncounter.nation;
		}
		...
		if (iEncounterType == ENCOUNTER_TYPE_BOAT)
		{
			...
			rFantom.SeaAI.Group.Name = sGName;
			Group_AddCharacter(sGName, rFantom.id);
			Log_TestInfo("Сгенерен потерпевший кораблекрушение");
			EmptyFantom_DropGoodsToSea(rFantom, iEncounterType);
			continue;
		}
		// <-- Ugeen
...
void Sea_LoginGroup(string sGroupID)
{
	...
	for (int i=0; i<iNumQuestShips; i++) //? homo;
	{
		...
		if (itmp != -1)
		{
            ...
            ProcessQuestShipSightLog(rCharacter);
...
void Sea_LoadIsland(string sIslandID)
{
	...
	if (iIslandIndex != -1 && Islands[iIslandIndex].visible == true)
	{
		...
		// трава на остров
		Sea_IslandDrawVegetation(sIslandID);
		...
		//TODO: ПОСЛЕ СШИТИЯ ОСТРОВОВ ЭТО ДОЛЖНО БЫТЬ ПЕРЕДЕЛАНО (см. прежде всего AIShip.c)
		aref arReloadFort;	
...
void Sea_IslandDrawVegetation(string sIslandID)
{
	DeleteGrass();
	int iIslandIndex = FindIsland(sIslandID);
	...
	if (iIslandIndex != -1 && Islands[iIslandIndex].visible == true)
	{
		...
		{
			float fW, fH, fMinDist, fMaxDist, fMinLod, fJungleScale = 10.0;
			fW = 20.0;
			fH = 200.0;
			fMinDist = 100.0;
			...
			CreateGrass("resource\models\islands\\"+ Islands[iIslandIndex].id +"\\"+ Islands[iIslandIndex].jungle.patch + ".grs", "Grass\\"+Islands[iIslandIndex].jungle.texture+".tga", fJungleScale, fW, fH, fMinDist, fMaxDist, fMinLod);
		}
	}
}
...
void Sea_CreateLighthouse(string _islandID)
{
	...
	if(_islandID == "Jamaica" || _islandID == "Cuba1" || _islandID == "Cuba2")
	{
		if(!IsDay())
		{
			...
			if(_islandID == "Cuba1")
			{
				// TODO Крутануть его - криво сейчас
...
float Sea_ApplyMaxSpeedZ(ref chref, aref arCharShip, float fWindDotShip) //float fTRFromSailDamage,
// arCharShip - корабль на НПС,  fTRFromSailDamage - паруса 0..1, fWindDotShip - направление ветра -1..1
{
	...
	fWindAgainstSpeed = stf(rShip.WindAgainstSpeed) * AIShip_isPerksUse(CheckCharacterPerk(chref, "GhostsGift"), 1.0, 1.06); // 1.7; мин fWindAgainstSpeed = 0.8 - мах 10.5
	if (fWindDotShip >= -0.1)
	{ //по ветру
...
string GetLoadingImage_Sea()
{
	...
	if(CheckAttribute(pchar, "nation"))
	{
		...
		{		        
			case ENGLAND:		
				imageName = "loading\sea_" + (1 + rand(2)) + ".tga";
			break;
			case FRANCE:		
				imageName = "loading\sea_" + (4 + rand(1)) + ".tga";
			break;
			case SPAIN:		
				imageName = "loading\sea_" + (6 + rand(1)) + ".tga";
			break;
			case HOLLAND:		
				imageName = "loading\sea_" + (8 + rand(2)) + ".tga";
			break;
			case PIRATE:		
				imageName = "loading\sea_" + (11 + rand(1)) + ".tga";
			break;
		}
	}
	return imageName;
