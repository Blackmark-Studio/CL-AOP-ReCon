...
bool LAi_group_Init()
{
	...
	LAi_group_SetAlarmReaction("LSC_NARVAL", LAI_GROUP_TmpEnemy, LAI_GROUP_FRIEND, LAI_GROUP_FRIEND);
	...
	LAi_group_Register("LSC_CASPER");
	LAi_group_SetLookRadius("LSC_CASPER", LAI_GROUP_GRD_LOOK);
	LAi_group_SetHearRadius("LSC_CASPER", LAI_GROUP_GRD_HEAR);
	LAi_group_SetSayRadius("LSC_CASPER", LAI_GROUP_GRD_SAY);
	LAi_group_SetPriority("LSC_CASPER", LAI_GROUP_PGUARDS);
	LAi_group_SetAlarmReaction("LSC_CASPER", LAI_GROUP_TmpEnemy, LAI_GROUP_FRIEND, LAI_GROUP_FRIEND);
	LAi_group_SetAlarmReaction("LSC_CASPER", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY, LAI_GROUP_NEITRAL);
	LAi_group_SetAlarmDown("LSC_CASPER", LAI_GROUP_PLAYER, 0.02);
			...
			if(idx >= 0)
			{
				...
				if(CheckAttribute(chr, "chr_ai.group"))
				{
					...
					if(selected)
					{
						...
						if(CheckAttribute(chr, "walker")) // затесавшиеся офицеры-ходоки с типом citizen
						{
							TEV.walkers = TEV.walkers + chr.id + ",";
							TEV.walkers.count = sti(TEV.walkers.count) + 1;
							RemoveCharacterEquip(chr, GUN_ITEM_TYPE); // чтобы не агрил стражу во время драки с ОЗГ
                            RemoveCharacterEquip(chr, MUSKET_ITEM_TYPE); // ~!mu
							if(group1 != LAI_GROUP_PLAYER) LAi_group_SetCheck(group1, "Officers_Citizen_Type");
							else LAi_group_SetCheck(group2, "Officers_Citizen_Type");
						}
						else // default
						{
...
int LAi_group_GetTarget(aref chr)
{
	...
	{
	    if (!LAi_group_IsEnemy(chr, &characters[index]))  // boal fix 19.08.06 ищет любых вооружённых НПС, а нужно врагов
		{
		    index = -1;
		}
	}
    if (index != -1 && characters[index].chr_ai.type == LAI_TYPE_INJURE) index = GetMainCharacterIndex();
	return index;
}
...
bool LAi_group_Attack(aref attack, aref hit)
{
	...
	//с актерами отношение не портим
	if (hit.chr_ai.group == LAI_GROUP_ACTOR)
        return false;
	//Нарвалы и Касперы мирняку урон не причиняют
	if (hit.chr_ai.group == LAI_GROUP_TmpEnemy && or(attack.chr_ai.group == "LSC_NARVAL", attack.chr_ai.group == "LSC_CASPER"))
        return false;
	...
	// Dolphin -> генератор "Охотник на торговцев"
	/*if(hit.chr_ai.group == "HOTP_HunterTrader_LandGroup" && !CheckAttribute(pchar, "HOTP.GenQuests.HunterTrader.quest.sound"))
	{
		PlaySound("Voice\Russian\EvilPirates01.wav");
		pchar.HOTP.GenQuests.HunterTrader.quest.sound = true;
	}*/

	// Французский сюжет, нападение без диалога на парней в пещере
	if(hit.chr_ai.group == "FL1_s33_group") pchar.MainQuest = "";
	// <-
...
void LAi_group_UpdateAlarm()
{
	...
	{
		TEV.AlarmFightTimer = stf(TEV.AlarmFightTimer) + GetRealDeltaTime();
		if (stf(TEV.AlarmFightTimer) > 1.0)
		{
		    TEV.AlarmFightTimer = 0.0;
			CheckAlarmFightTutorials();
		}
	}
}