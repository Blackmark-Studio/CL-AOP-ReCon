...
int _GetDistanceToColony2D(string _sColony)
{
	...
	if(_sColony == "FortOrange") sColonyTown = "Shore35";
	if(_sColony == "LaVega") sColonyTown = "LaVega_town";
	if(_sColony == "LeFransua") sColonyTown = "LeFransua_town";
...
void InitInterface(string iniName)
{
	...
	for(int i=0; i<MAX_COLONIES; i++)
	{
		...
		if(sColony == "Providence") continue;
...
void IDoExit(int exitCode)
{
	...
	if (CheckAttribute(&TEV, "BackToItems"))
	{
		DeleteAttribute(&TEV, "BackToItems");
...
void FillTable()
{
					...
					if(fMouseY >= stf(GameInterface.MAP.imagelist.(sColony).y - 10))	// 0
					{
						if(fMouseY <= stf(GameInterface.MAP.imagelist.(sColony).y) + 60.0)	// 50
						{
							if(sColony != "Panama" && sColony != "IslaMona" && sColony != "KhaelRoa" && sColony != "Caiman")
							{
								...
								{
									if (GetArealByLocation(&locations[z]) == island)
									{
										row = "tr" + n;
	...
	if (colonyindex != i)
	{
		...
		{
			setCharacterShipLocation(pchar, "Providencia_town");
			setWDMPointXZ("Providencia_town");
			ssColony = "Providence";
		}*/
		if (ssColony == "") return;
		XI_WindowShow("TP_WINDOW", true);
		...
		{
			if (GetArealByLocation(&locations[y]) == ssColony || CheckForIslandID(&locations[y]) == ssColony)
			{
				row1 = "tr" + x;
...
void SelectRColony()
{
	...
	if (bBettaTestMode)
	{
		...
		if (fMouseX >= 600.0 && fMouseX <= 680.0 && fMouseY >= 1090.0 && fMouseY <= 1180.0)
		{
		...
		{640.1107788/1136.06
			setCharacterShipLocation(pchar, "Providencia_town");
			setWDMPointXZ("Providencia_town");
			DoQuestReloadToLocation("Providencia_town", "reload", "reload1", "");
		}*/
	}
}
...
void ShowColonyInfo(int iColony)
{
	...
	for(iType=0; iType<2; iType++)
	{
		...
		for(i=0; i<TRADEASSISTANT_MAXGOODS; i++)
		{
			...
			SendMessage(&GameInterface,"lslslllll",MSG_INTERFACE_MSG_TO_NODE,"GOODS_PICTURES", 0, Goods[a_iMax[i]].name, argb(255, 128, 128, 128), iGoods_x,iGoods_y,iGoods_x+28,iGoods_y+28);
			iGoods_y+=28;
...
int GetMaxFortCannons(string _FortCommander)
{
	...
	switch(_FortCommander)
	{
		...
		case "Providencia Fort Commander": _iCannons = 80; break;
	}
...
float _GetDirToColony(string _sColony)
{
	...
	if(_sColony == "FortOrange") sColonyTown = "Shore35";
	if(_sColony == "LaVega") sColonyTown = "LaVega_town";
	if(_sColony == "LaFransua") sColonyTown = "LeFransua_town";
