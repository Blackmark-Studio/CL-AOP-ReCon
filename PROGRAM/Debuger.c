...
void ActiveF5Control()
{
	...
	Log_TestInfo("Дамп аттрибутов ближайшего персонажа");
}
...
void ActiveF7Control()
{
    ...
    if(characterIndex != -1)
	{
		...
		if(!LAi_IsImmortal(characterRef)) // Квестовые
		{
			...
			Log_TestInfo("Убили персонажа с ID == " + characterRef.Id);
		}
		else
		{
			Log_TestInfo("Нельзя убить персонажа с ID == " + characterRef.Id);
		}
	}
}
...
void ActiveF12Control()
{
    ...
    if (res != -1)
    {
        ...
        Log_SetStringToLog("" + GetFullName(findCh) +
                           " "+XI_ConvertString("Rank")+" " + findCh.rank + " "+XI_ConvertString("Health")+" "+res + "/" + LAi_GetCharacterMaxHP(findCh) /*+ " Индекс: " + findCh.index*/);
