...
string CreateMerchant(int ination)
{
    ...
    chref.mapEnc.Name = LowerFirst(XI_ConvertString("Merchant"));
...
void SetMerchantShip(ref Cap, int igoods)
{
    ...
    if  (killM >= killMax && rand(1))
    {
        ...
        Cap.dialog.filename = "Capitan_dialog.c";
...
string All_GetColony()
{
    ...
    if (locnum != -1 )
    {
		...
		else // а если в джунглях или на побережье?
		{
			...
			{
				return GetCityNameByLocation(&locations[locnum]);
			}	
			else // остров с одним городом
			{	
				return GetCityNameByAreal(locations[locnum].islandId);
			}	
		}
    }
    if (!IsEntity(&worldMap)) // в море
    {
        return Sea_FindNearColony(true); //Rosarak. Исправил функцию
    }
    return "none";
}
...
string GenerateMerchant()
{
    ...
    if ( MerchantPrepare(iNation) )
    {
        ...
        reztext = GetConvertStr(tid, "RumourTexts.txt");
        MerPrm.ination = MerPrm.nation; // fix
        MerPrm.nation = NationNameGenitive(sti(MerPrm.nation));
        MerPrm.QuestGoodsIdx = GetGoodsNameAlt(sti(MerPrm.QuestGoodsIdx));
