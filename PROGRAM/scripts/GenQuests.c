...
void Rat_GenerateRat()
{
	...
	for(int i=1; i<=iRatCount; i++)
	{
		...
		rChar.Dialog.FileName = "Quest\GenQuests_dialog.c";
...
void ContraDeliverQuest_ReloadConterToRoom(string sQuest) // генерим клона контрабандиста
{
	...
	rChar.Name = UpperFirst(XI_ConvertString("WhoSmuggler"));
	rChar.LastName = "";
...
void ContraDeliverQuest_ContraIsDead(string sQuest)
{
	...
	rChar.model = "citiz_" + (rand(9)+21); //watch_quest_moment; Меняем модель (другой пришёл)
	FaceMaker(rChar);
...
void ContraDeliverQuest_GeneratePatrolToRoom()
{
	...
	for(int i=0; i<=2; i++)
	{
		...
		SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
		rChar.Name = XI_ConvertString("Officer");
		rChar.LastName = "";
...
void ContraMeetManQuest_EnterToShore(string sQuest) // Вошли в бухту
{
	...
	{
		rChar = GetCharacter(NPC_GenerateCharacter("ContraMeetManQuest_QuestMan", GetRandSubString(COMMONERS), "man", "man", sti(PChar.rank)*2, PIRATE, -1, true)); //watch_quest_moment; вестовой
		SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
	...
	else // Солдеры
	{
		...
		for(int i=0; i<=iSoldCount; i++)
		{
			...
			if(i==0) // Офицер
			{
				rChar = GetCharacter(NPC_GenerateCharacter("ContraMeetManQuest_Officer_" + i, "off_"+sShortNation+"_6", "man", "man", sti(PChar.rank)*2, iNation, -1, true));
				SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
				rChar.Name = XI_ConvertString("Officer");
				rChar.LastName = "";
			}
			else // Солдаты
			{
				...
				rChar.Name = XI_ConvertString("Soldier");
				rChar.LastName = "";
			}
...
void ContraMeetManQuest_TimeIsLeft(string sQuest) // Вышло время
{
	...
	AddQuestUserData(sQuestTitle, "sDestination", XI_ConvertString(PChar.GenQuest.ContraMeetMan.ShoreID + "Dat")); //TODO: падеж
	CloseQuestHeader(sQuestTitle);
...
void ContraMeetManQuest_ReloadConterToRoom(string sQuest)
{
	...
	for(int i=0; i<=1; i++)
	{
		rChar = GetCharacter(NPC_GenerateCharacter("ContraMeetManQuest_Contrick_" + i, "Pirate_"+(rand(15)+1), "man", "man", sti(PChar.rank)*2, PIRATE, 0, true));
		SetFantomParamFromRank(rChar, sti(PChar.rank)*2, true);
		rChar.Name = UpperFirst(XI_ConvertString("WhoSmuggler"));
		rChar.LastName = "";
		rChar.Greeting = "Gr_Smuggler Agent";
...
void CitizenFindItemMiniQuest_Timer(string sQuest)
{
	...
	{
	    AddQuestRecord("SeekCitizenRing", "3");
	    AddQuestUserData("SeekCitizenRing", "sSex", GetSexPhrase("","ла"));
		RemoveItems(PChar, "WeddingRing", 1);
		DeleteAttribute(&items[FindItem("WeddingRing")], "City");
	}
	else
	{
	    AddQuestRecord("SeekCitizenRing", "4");
	    ref ring = ItemsFromID("WeddingRing");
	    ring.shown = false;
		DeleteAttribute(ring, "City");
	}
...
//=====================================================================================================================================
// KZ > единая зачистка церковного квеста №1
void Church_GenQuest1_ClearEntities()
{
...
// KZ > провал квеста по времени
void Church_GenQuest1_Timer(string sQuest)
{
	if (!CheckAttribute(PChar, "GenQuest.ChurchQuest_1")) return;
	string sTitle = PChar.GenQuest.ChurchQuest_1.QuestTown + "ChurchGenQuest1";
	RemoveItems(PChar, "ChurchManuscript", 1); // > если рукопись уже забрали - изымаем
	Church_GenQuest1_ClearEntities();
	AddQuestRecordEx(sTitle, "ChurchGenQuest1", "11");
...
void Church_GenQuest1_ChangeCapitanLocation(string sQuest)
{
	...
	// > Кэп-пропойца ждёт в таверне - сажаем его при заходе
...
void Church_GenQuest1_DeleteCapitan(string sQuest)
{
	if (GetCharacterIndex("ChurchGenQuest1_Cap") != -1)
	{
		CharacterFromID("ChurchGenQuest1_Cap").LifeDay = 0;
	}
	Group_DeleteGroup("ChurchGenQuest1_CapGroup");
	PChar.Quest.Church_GenQuest1_ChangeCapitanLocation.over = "yes";
}
...
void Church_GenQuest2_GenerateBandits(string sQuest)
{
	...
	{
		ref rChar = GetCharacter(NPC_GenerateCharacter("Church_GenQuest2_Bandit_" + i, "pirate_" + (rand(15)+1), "man", "man", iGenRank, PIRATE, -1, true));
		SetNPCModelUniq(rChar, "pirate", MAN);
		if(i == 0) // Главарь банды
		{
			...
			if(!ENCYCLOPEDIA_DISABLED && CheckAttribute(pchar, "GenQuest.ChurchQuest_2.QuestGangJungle") && CheckAttribute(pchar, "GenQuest.ChurchQuest_2.WayOfShortVariant"))
			{
				if(CheckRandomPage("questjungle", "churchquest", 1))
				{
					rChar.encyclopedia.tome = GetRandomTome();
					rChar.encyclopedia.page = 5;
				}
			}
			if(CheckAttribute(pchar, "GenQuest.ChurchQuest_2.BanditsInShore"))
			{
				if(CheckRandomPage("questjungle", "churchquest", 2))
				{
					rChar.encyclopedia.tome = GetRandomTome();
					rChar.encyclopedia.page = 5;
				}
...
//		ChangeCharacterAddressGroup(rChar, location.id, "goto", "goto" + (i + 1));
	...
	// > банду нашли - маркеры "уже опрошен" на горожанах больше не нужны, чистим (иначе висят вечно)
	Church_GenQuest2_ClearAskedPeople(PChar.GenQuest.ChurchQuest_2.QuestTown);
	// > страховка от софт-лока, когда игрок мог уйти из локации, не завершив сцену
	SetFunctionExitFromLocationCondition("Church_GenQuest2_LeaveGangLocation", pchar.location, false);
}

// > снять с горожан маркер "уже опрошен"
void Church_GenQuest2_ClearAskedPeople(string sTown)
{
	for (int i = 0; i < TOTAL_CHARACTERS; i++)
	{
		ref chr = GetCharacter(i);
		if (CheckAttribute(chr, "GenQuest.ChurchQuest_2.AskedPeople"))
			DeleteAttribute(chr, "GenQuest.ChurchQuest_2.AskedPeople");
	}
}

// > Удаление квестовых бандитов по церковному генератору №2
void Church_GenQuest2_RemoveBandits()
{
	int i;
	string sId;
	ref rChar;
	...
	for (i = 0; i < 10; i++)
	{
		sId = "Church_GenQuest2_Bandit_" + i;
		if (GetCharacterIndex(sId) == -1) continue;
		rChar = CharacterFromID(sId);
		ChangeCharacterAddressGroup(rChar, "None", "", "");
		rChar.LifeDay = 0;
	}
...
// > На случай выхода из локации с бандой без взаимодействия с ними
void Church_GenQuest2_LeaveGangLocation(string sQuest)
{
	chrDisableReloadToLocation = false;
...
void ReasonToFast_MeetPatrolShore(string qName) // генерация патруля
{
	...
	int num, i, iNation, n, iPatrolOfficer;
	...
	for (i = 0; i < num; i++)
	{
		if (i == 0)
		{					
			iPatrolOfficer = NPC_GenerateCharacter(str + i, "off_"+NationShortName(iNation)+"_"+(rand(1)+5), "man", "man", iRank + 2, iNation, 1, true);
...
void ReasonToFast_GenerateHunter() // генерация ОЗГ
{
	...
	chr = GetCharacter(NPC_GenerateCharacter("ReasonToFast_Hunter", "citiz_"+(rand(9)+51), "man", "man", iRank, PIRATE, -1, true)); //watch_quest_moment; где CoolFighter я ставил наёмников
...
int ReasonToFast_GetVictimShipType()
{
	...
	{	
	    case 0: 		 
			...
			sTemp = XI_ConvertString("Barque");
		break;
	    case 1:  
			iShipType = SHIP_FLEUT;
			sTemp = XI_ConvertString("Fleut");
		break;
	    case 2: 
			iShipType = SHIP_GALEON_L;         
			sTemp = XI_ConvertString("Galeon_l");
		break;
	    case 3: 
			iShipType = SHIP_PINNACE; 			
			sTemp = XI_ConvertString("Pinnace");
		break;  	
		case 4:
			iShipType = SHIP_CARACCA;
			sTemp = XI_ConvertString("Caracca");
		break;
	}
...
void ReasonToFast_GenerateVictimShip(int iShipType, int iShipGoods) // генерация призового корабля
{
	...
	ref chref = GetCharacter(NPC_GenerateCharacter("VictimCap_1", "officer_"+(rand(9)+11), "man", "man", 25, iNation, -1, true)); //watch_quest_moment; исходя из класса генерируем средняка, но можно и наёмников ставить
...
void ReasonToFast_PreparePirateShip(string qName) // генерация пиратского корабля
{
	...
	{	
	    case 0: 		 
			...
			sTemp1 = LowerFirst(XI_ConvertString("WhoSmuggler"));
		break; 		
	    case 1:  
			iShipType = SHIP_BRIG;     		
			sTemp1 = LowerFirst(XI_ConvertString("Brig"));
		break; 		
	    case 2:  
			iShipType = SHIP_BRIGANTINE;
			sTemp1 = LowerFirst(XI_ConvertString("Brigantine"));
		break;
	    case 3: 
			iShipType = SHIP_CORVETTE; 	
			sTemp1 = LowerFirst(XI_ConvertString("Corvette"));
		break;
	    case 4: 
			iShipType = SHIP_FRIGATE;         
			sTemp1 = LowerFirst(XI_ConvertString("Frigate"));
		break;
	    case 5: 
			iShipType = SHIP_LINESHIP; 			
			sTemp1 = LowerFirst(XI_ConvertString("LineShip"));
		break;  	
	}
		
	Log_QuestInfo("Генерим пиратский корабль на глобалке.");
	
	sld = GetCharacter(NPC_GenerateCharacter("PirateCapt", "citiz_"+(rand(9)+51), "man", "man", iRank, PIRATE, 30, true)); //watch_quest_moment
	sld.dialog.filename      = "Quest\GenQuests_dialog.c";
	sld.dialog.currentnode   = "ReasonToFast_FightCap";
	sTemp = GenerateRandomNameToShip(PIRATE);
	FantomMakeCoolSailor(sld, iShipType, sTemp, CANNON_TYPE_LBS_BY_SHIP, 50+rand(40), 50+rand(40), 50 + rand(40));
...
//        Log_Info(StringFromKey("InfoMessages_105"));
		...
		itmName = "incas_collection"; // генерим сокровища
...
void BurntShipQuest_TimeIsOver(string _quest)
{
	...
	character.model = "portman_" + (rand(15) + 1);
...
{
	int i, index, qty = sti(PChar.GenQuest.PiratesOnUninhabited.PiratesQty);
...
{
	int i, qty = sti(PChar.GenQuest.PiratesOnUninhabited.PiratesQty);
...
{
	aref arQ; makearef(arQ, PChar.GenQuest.PiratesOnUninhabited);
	ref location = &Locations[FindLocation(arQ.TreasureShore)];
	ref character;
	string boxId = arQ.TreasureBox;
	string attribute;
	aref boxItems;
	int i = 0;
	int count, temp, rank;
...
//	Log_Info(StringFromKey("InfoMessages_105"));
	...
	if(!CheckAttribute(arQ, "TreasureLose"))
	{
		count = 2 + makeint(MOD_SKILL_ENEMY_RATE / 3) + dRand(1);

		arQ.ShorePiratesQty = count;
		...
		while(i < count)
		{
			...
			{
				arQ.ShorePiratesQty = i;
				break;
			}
			
			attribute = "l" + (i + 1);
			
			character = GetCharacter(NPC_GenerateCharacter("PirateOnUninhabited_ShorePirate" + i, "pirate_"+(rand(15)+1), "man", "man", rank, PIRATE, -1, true));
			SetNPCModelUniq(character, "pirate", MAN);
			SetFantomParamFromRank(character, rank, true);
			ChangeCharacterAddressGroup(character, location.id, "goto", "goto" + (i + 1));
			character.dialog.filename = "Quest\GenQuests_dialog.c";
		...
		PChar.Quest.PiratesOnUninhabited_ShoreTreasureLose.function = "PiratesOnUninhabited_ShoreTreasureLose";
		...
		if(rand(2) == 1)
		{
			boxItems.incas_collection = 1 + rand(4);
		}
	...
	else
	{
		...
		PChar.Quest.PiratesOnUninhabited_ClearQuest.win_condition.l1.Location = arQ.TreasureShore;
...
void PiratesOnUninhabited_ShoreTreasureLose(string _quest)
{
	...
	int i, index, qty = sti(PChar.GenQuest.PiratesOnUninhabited.ShorePiratesQty);
	...
	DeleteAttribute(location, boxId + ".QuestClosed");
...
void PiratesOnUninhabited_ClearQuest(string _quest)
{
	...
	AddQuestUserData(title, "shore", LowerFirst(XI_ConvertString(PChar.location + "Gen")));
	CloseQuestHeader(title);

	PChar.Quest.PiratesOnUninhabited_OpenChest.over = "yes";
	
	DeleteAttribute(PChar, "GenQuest.PiratesOnUninhabited");
}

void DeadlineHasExpired(string qName) // лесник если просрочили поиск корабля
{
	PChar.GenQuest.ShipWreck_BadPirate.ClearShip = true;
	Map_ReleaseQuestEncounter("ShipWreck_BadPirate"); //лесник. убираем с глобалки
	...
	if (IsEntity(&worldMap) || bSeaActive)
	{
        ...
        pchar.quest.qazwsx.function = "PiratesRemove";
        Log_Info(StringFromKey("InfoMessages_106"));
	}
    else
	{
        ref sld = CharacterFromID("ShipWreck_0");
        RemovePassenger(Pchar, sld);
        sld.lifeday = 0;
        Log_Info(StringFromKey("InfoMessages_107"));
        PlaySound("Notebook_1");
	}
	DeleteAttribute(PChar, "GenQuest.ShipWreck");
}

void PiratesRemove(string qName)//удалить негоцианта  лесник
{
	ref sld = CharacterFromID("ShipWreck_0");
	RemovePassenger(Pchar, sld);
	sld.lifeday = 0;
	Log_Info(StringFromKey("InfoMessages_108"));
	PlaySound("Notebook_1");
...
void PiratesOnUninhabited_ShipSink(string _quest)
{
	...
	PChar.Quest.PiratesOnUninhabited_ShipDeadline.over = "yes"; // > Корабль утоплен - дедлайн больше не нужен
...
// KZ > Дедлайн на перехват корабля пирата.
// > Если игрок так и не перехватил его за отведённый срок, то тихо закрываем эту ветку, чтобы сам квест не висел вечно
void PiratesOnUninhabited_ShipDeadline(string _quest)
{
	ref sld = CharacterFromID("PirateOnUninhabited_0");
	...
	CloseQuestHeader("PiratesOnUninhabited" + PChar.GenQuest.PiratesOnUninhabited.StartShore);
	...
	Log_TestInfo("PiratesOnUninhabited_ShipDeadline > корабль пирата не перехвачен в срок - ветка захвата закрыта");
}
//=====================================================================================================================================
...
void RemoveCrewEpidemy_GenQuest()
{
	...
	Log_SetStringToLog(StringFromKey("InfoMessages_109"));
	for(i = 0; i < COMPANION_MAX; i++)
	{
		...
		if(cn >= 0)
		{
			...
			Log_SetStringToLog(StringFromKey("InfoMessages_110", chref.Ship.Name, FindPeoplesString(crew, "No")));
...
{
	if (CheckCharacterPerk(pchar, "Sanitary") || GetCharacterSPECIAL(pchar,"LUCK") * 10 > rand(120)) // шутник блин
	{
...
{
	if (CheckCharacterPerk(pchar, "Sanitary")) return;
	int DayQty;
...
void ShipSituation_MakeDetonate(string qName)
{
	...
	string shipSex = "";
	if (LanguageGetLanguage() == "russian" || LanguageGetLanguage() == "polish")
		shipSex = GetShipSexWord(rBaseShip.BaseName, "", "a");
	Notification(StringFromKey("InfoMessages_111", XI_ConvertString(rBaseShip.BaseName), sld.Ship.Name, shipSex), "Brander");
...
void CheckQuestAboardCabinSituation(ref echar)
{
	...
	{	
		echar.FaceId = 333;
		echar.name = FindPersonalName("EmptyShip_name");
		echar.lastname = "";
		bQuestDisableMapEnter = true;
	...
	// квест жемчужников
	if(CheckAttribute(echar, "PearlTartane") && sti(echar.PearlTartane))
    {
        int pinctadaQty = GetSquadronGoods(pchar, GOOD_PINCTADA);

        echar.PearlTartane = false; //уже ограбили
        pchar.questTemp.Sharp.PearlAreal.PinctadaQty = pinctadaQty;
        ...
        {
            pchar.questTemp.Sharp.PearlAreal.qtyShipsSuc = 0;
        }
        pchar.questTemp.Sharp.PearlAreal.qtyShipsSuc = sti(pchar.questTemp.Sharp.PearlAreal.qtyShipsSuc) + 1;
...
void EncGirl_Saved()
{
	...
	sld.dialog.filename = "Encounters\RapersGirl.c";
...
void EncGirl_GenQuest_GetCoins(string qName)
{
	...
	sld.lifeDay = 0;
	AddSimpleRumour(StringFromKey("Common_rumours_137", pchar.GenQuest.EncGirl.name), sti(sld.nation), 5, 1);
...
{
	SetEventHandler("frame", "EncGirlFightMode", 0);
	chrDisableReloadToLocation = true; //HardCoffee наше дело телячье: сказали лочить - значит лочим!
	ref sld = characterFromId("CangGirl");
	if (sld.id == "none") return;
	...
	switch (pchar.GenQuest.EncGirl)
	{
		...
		break;
		case "ChestGetted":
			//chrDisableReloadToLocation = true; //установлена закрывашка для всех случаев
			LAi_type_actor_Reset(sld);
			sld.dialog.currentnode = "Node_132_18";
			pchar.quest.EncGirl_DeathAgain.over = "yes";
		break;
		pchar.quest.EncGirl_DeathSimple.over = "yes"; //fix
	}
	LAi_SetActorType(pchar);
...
void EncGirlFightMode() //HardCoffee закрытие квеста с заблудшей девкой, если начались тёрки со стражей у ворот
{	//Это для варианта с сопровождением в город. Для вариантов с сопровождеием к отцу или сундуку - есть отдельный "EncGirl_Massacre"
	if (LAi_grp_playeralarm <= 0) return;
	DelEventHandler("frame", "EncGirlFightMode");
	pchar.quest.EncGirl_DeliveBack.over = "yes";
	ref rChr = CharacterFromId("CangGirl");
	if (rChr.id == "none") return;
	string sTemp = "";
	float locx, locy, locz;
	...
	else
	{
		...
		LAi_ActorRunToLocation(rChr, "reload", sTemp, "none", "", "", "OpenTheDoors", -1.0);
	}
	rChr.lifeDay = "1";
	rChr.location = "";
...
void EncGirl_MeetRapers(string qName)
{
	...
	for(int i = 1; i <= 3; i++)
	{
		...
		LAi_type_actor_Reset(sld);
		sld.dialog.filename = "Encounters\Rapers.c";
...
void EncGirl_TreasureLose(string qName)
{
	...
	DeleteAttribute(location, boxId + ".items");
	DeleteAttribute(location, boxId + ".Treasure");
	location.(boxId).money = 0;

	DeleteAttribute(pchar, "GenQuest.TreasureEncGirl"); 	// > эти два атрибута
	DeleteAttribute(pchar, "GenQuest.TreasureEncGirlBox");	// < жили в сейве вечно
	DeleteAttribute(pchar, "GenQuest.EncGirl");
}
...
void EncGirl_SetBerglar(string qName)
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("Berglar_EncGirl", "citiz_"+(rand(9)+51), "man", "man", iTemp, PIRATE, -1, true)); //watch_quest_moment
...
void EncGirlFack_outRoom()
{
	...
	ref sld = characterFromId("Berglar_EncGirl");
	sld.LifeDay = 0;
	LAi_SetActorType(sld);
...
void EncGirl_GenQuest_GetBag(string qName)
{
	...
	LocatorReloadEnterDisable(pchar.GenQuest.EncGirl.city + "_tavern", "reload2_back", false); // > за EncGirl_ExitTavernRoom, которая уже не сработает
	pchar.quest.EncGirl_ExitRoom.over = "yes"; // > иначе сработает по удалённому атрибуту
...
void CaptainComission_GenerateSituation(ref chref)
{
	if(CheckAttribute(chref, "RealEncounterType") && sti(chref.RealEncounterType) != ENCOUNTER_TYPE_ALONE && !CheckAttribute(pchar,"GenQuest.CaptainComission"))
	{
		...
		if (CheckAttribute(chrShip, "Mode") && chrShip.Mode == "trade")
		{
			...
			{
				int iRealShip = SHIP_NOTUSED;
				int iShipType = -1;

				if (CheckAttribute(chrShip, "Type"))
					iRealShip = sti(chrShip.Type);

				if (iRealShip >= 0 && iRealShip < REAL_SHIPS_QUANTITY)
				{
					if (CheckAttribute(&RealShips[iRealShip], "BaseType"))
						iShipType = sti(RealShips[iRealShip].BaseType);
				}
				...
				if (iShipType >= 0)
				{
					...
					trace("CaptainComission_GenerateSituation: у '" + chref.id + "' не определить тип корабля, генератор пропущен");
			}
		}	
	}
}
...
{
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_1" , "citiz_"+(rand(9)+1), "man", "man", 10, sti(pchar.GenQuest.CaptainComission.Nation), -1, true)); //done Konstrush
...
void CaptainComission_30DaysIsLeft(string qName) // прошло 30 дней после разговора в таверне
{
	...
	if(pchar.GenQuest.CaptainComission.GetSlave == "NoSlave")
	{
		...
		if(pchar.GenQuest.CaptainComission.AfterTavernSpeak == "goodSpeak")
		{
			...
			AddQuestUserData("CaptainComission1", "sSex", GetSexPhrase(StringFromKey("SexPhrase_21"),StringFromKey("SexPhrase_22")));
...
{
	string sCurIsland = Islands[GetCharacterCurrentIsland(pchar)].id;
	string sIsl = GetRandomIslandExcept("Panama,Pearl,Tenotchitlan");
	while(sIsl == sCurIsland)
	{
		sIsl = GetRandomIslandExcept("Panama,Pearl,Tenotchitlan");
	}
	string sShore = GetIslandRandomFreeShoreId(sIsl);
	while(sShore == "")
	{
		sIsl = GetRandomIslandExcept("Panama,Pearl,Tenotchitlan");
		sShore = GetIslandRandomFreeShoreId(sIsl);
		if (!isLocationFreeForQuests(sIsl)) sShore = "";
	}
	pchar.GenQuest.CaptainComission.Island = sIsl;
	pchar.GenQuest.CaptainComission.Island.Shore = sShore;
}
...
void CaptainComission_GenerateSlave(string qName)
{
	...
	int iNation = GetCityNation(GetSubStr(pchar.GenQuest.CaptainComission.toPlantator, "_", 0));
	string slai_group = GetNationNameByType(iNation)  + "_citizens";
	
	string model[7];
	model[0] = "citiz_45";
	model[1] = "blacksmith_1";
	model[2] = "barmen_1";
	model[3] = "barmen_9";
	model[4] = "citiz_46";
	model[5] = "shipowner_7";
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_Slave", model[rand(6)], "man", "man", rand(5) + 5, sti(pchar.GenQuest.CaptainComission.Nation), -1, true));	
	...
	if(pchar.GenQuest.CaptainComission.GetSlave == "NoSlave")	
	{
		...
		LAi_LocationFightDisable(&Locations[FindLocation(pchar.GenQuest.CaptainComission.toPlantator)], true);
		LAi_SetFightMode(pchar, false);
		LAi_LockFightMode(pchar, true);
		sld.dialog.currentnode = "CaptainComission_70";
		ChangeCharacterAddressGroup(sld, pchar.GenQuest.CaptainComission.toPlantator, "quest", "sit2");
		LAi_SetActorType(sld);
		...
		for (int i = 0; i < 3; i++)
		{
            if(iNation != PIRATE && LAi_IsCapturedLocation == 0) iChar = NPC_GeneratePhantomCharacter("milit_sold", iNation, MAN, 2);
            else iChar = NPC_GeneratePhantomCharacter("pirate", iNation, MAN, 2);
            ...
            if(i < 2) 	ChangeCharacterAddressGroup(sld, pchar.GenQuest.CaptainComission.toPlantator, "quest", "sit"+(2*i+1));
			else		ChangeCharacterAddressGroup(sld, pchar.GenQuest.CaptainComission.toPlantator, "goto", "goto19");
			sld.dialog.filename = "Quest\GenQuests_dialog.c";
			sld.dialog.currentnode = "CaptainComission_80";				
        }
        DeleteAttribute(pchar,"GenQuest.CaptainComission.toPlantator");
...
void CaptainComission_GeneratePassengerSlave()
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_Slave", model[rand(2)], "man", "man", rand(5) + 5, sti(pchar.GenQuest.CaptainComission.Nation), -1, true));	
	sld.city = pchar.GenQuest.CaptainComission.City;	
	sld.name = pchar.GenQuest.CaptainComission.SlaveName;
	sld.lastname = "";	
...
void CaptainComission_GuardsAfter(string qName)
{
	...
	if (!LAi_IsDead(sld))
	{
		...
		pchar.quest.CaptainComission_SlaveOnMayak.win_condition.l1.location = pchar.GenQuest.CaptainComission.SlaveEscapeLocation;
		pchar.quest.CaptainComission_SlaveOnMayak.function = "CaptainComission_SlaveEscape";
...
void CaptainComission_SlaveEscape(string qName)
{
	aref grp;	
	ref pLoc, sld;
	string encGroup, str, locator;
	...
	pLoc = &locations[FindLocation(pchar.GenQuest.CaptainComission.SlaveEscapeLocation)];
	encGroup = LAi_FindRandomLocator("encdetector");
	str = "locators." + encGroup;
	makearef(grp, pLoc.(str));
...
void CaptainComission_ReturnPlantatorDialogNode()
{
    if (CheckAttribute(pchar, "GenQuest.CaptainComission.plantatorId"))
	{
		ref sld = CharacterFromId(pchar.GenQuest.CaptainComission.plantatorId);
		sld.dialog.currentnode = "First time";
		sld.dialog.TempNode = "First time";
		DeleteAttribute(pchar, "GenQuest.CaptainComission.plantatorId");
	}
}
...
void CaptainComission_ExitFromMayak(string qName)
{
	...
	sld = characterFromId("CapComission_Slave");
	string sSlaveState;
	if (!LAi_IsDead(sld)) 	sSlaveState = "GetSlave";
	else 					sSlaveState = "SlaveDied";
	pchar.GenQuest.CaptainComission.GetSlave = sSlaveState;

	if(sSlaveState == "GetSlave")
	{
...
void GetSlaveSpeciality()
{
    ...
    {
        case 0 :
             Speciality = XI_ConvertString("SpecialityShipbuilding");
        break;
        case 1 :
            Speciality = XI_ConvertString("SpecialityArtillery");
        break;
        case 2 :
            Speciality = XI_ConvertString("SpecialityNavigation");
        break;
    }
...
void CaptainComission_GeneratePirateShips(string qName)
{
	...
	for (i=1; i<=2; i++)
	{
		...
		if(makeint(pchar.rank) > 13) { iShipRank = rand(2) + 3; }
		if(makeint(pchar.rank) > 6 && makeint(pchar.rank) < 14) { iShipRank = rand(2); }
		if(makeint(pchar.rank) < 7) { iShipRank = rand(1); }
		switch (iShipRank)
		{
			case 0: 		 
		...
		sld = GetCharacter(NPC_GenerateCharacter("CaptainAttack_"+i, "citiz_"+(rand(9)+51), "man", "man", Rank, PIRATE, 3, true));
		//watch_quest_moment; при желании здесь и выше можно через sTemp2 брать и officer, и citiz в зависимости от корабля
...
void CaptainComission_TimeIsOver(string qName)
{
	...
	if(CheckAttribute(pchar,"GenQuest.CaptainComission.variant"))
	{
		...
		{
			pchar.quest.CaptainCommission_SailToShore.over = "yes";
...
void CaptainComission_GenerateShip()
{
	...
	ref chref = GetCharacter(NPC_GenerateCharacter("CapComission_1", "navy_off_"+NationShortName(iNation)+"_"+(rand(3)+1), "man", "man", iRank, iNation, -1, true));
	
	chref.name = pchar.GenQuest.CaptainComission.Name;
	chref.lastname = "";
	chref.city = pchar.GenQuest.CaptainComission.City;
	chref.dialog.filename      = "Quest\GenQuests_dialog.c";
	...
	FantomMakeCoolSailor(chref, sti(pchar.GenQuest.CaptainComission.ShipType), chref.Ship.Name, CANNON_TYPE_LBS_BY_SHIP, 90, 90, 90);
	FantomMakeCoolFighter(chref, makeint(pchar.rank) + rand(10) + 5, 90, 50, LinkRandPhrase("blade22","blade34","blade21"), "pistol3", 80);
	chref.cirassId = FindItem("cirass5");  
	...
	int IslandIdx = FindIsland(GetArealByCityName(pchar.GenQuest.CaptainComission.City));
	ref pLoc = &Islands[IslandIdx];
	float x, y, z;
	FindIslandLocatorXYZ(IslandIdx, "reload1", &x, &y, &z);
...
void CaptainComission_ExitFromLoc(string qName)
{
	...
	if(pchar.GenQuest.CaptainComission == "close")
	{
		...
		{
			sld = CharacterFromID("CapComission_1");
			sld.Dialog.Filename = "Capitan_dialog.c"; // при повторном диалоге
			SetCapGreetByModel(sld);
...
void CaptainComission_toShore() // вышвыриваем в бухту без штанов
{
	...
	AddSimpleRumour(RandPhraseSimple(StringFromKey("Common_rumours_140"),
				StringFromKey("Common_rumours_141")), sti(pchar.GenQuest.CaptainComission.Nation), 5, 1);
...
void CaptainCommission_SailToShore(string qName)
{
	...
	SetFunctionTimerCondition("CaptainCommission_SailToShoreOver", 0, 0, 1, false); // таймер лесник
}

void CaptainCommission_SailToShoreOver(string qName) // мой код . лесник. если опоздали квест удаляется .
{
    Group_DeleteGroup("Sea_CapComission_1");
    ref sld = CharacterFromID("CapComission_1");
    sld.LifeDay = 0;
...
void CaptainComission_MeetInShore(string qName)
{
	...
	pchar.quest.CaptainCommission_SailToShoreOver.over = "yes"; // снятие таймера - лесник
...
void CaptainComission_GenerateShorePatrol()
{
	...
	int i, iQty = makeint(2 * GetOfficersQuantity(pchar) + 3);
	pchar.GenQuest.CaptainComission.SoldierQty = iQty;
	for (i = 2; i <= iQty; i++)
	{
...
void CaptainComission_AfterShoreBattle(string qName)
{
	...
	if (LAi_IsDead(sld))
	{
		...
		AddQuestUserData("CaptainComission2", "sShoreName", XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Pre"));
		AddQuestUserData("CaptainComission2", "sName", pchar.GenQuest.CaptainComission.Name);
		CloseQuestHeader("CaptainComission2");	
		DeleteAttribute(pchar, "GenQuest.CaptainComission");	
	}
...
void CaptainComission_GangExit()
{
	...
	LAi_SetPlayerType(pchar);
	GetCharacterPos(pchar, &locx, &locy, &locz);
	int i, nGang = sti(pchar.GenQuest.CaptainComission.GangNum);
	string sReload = LAi_FindNearestLocator("reload", locx, locy, locz);
    for (i = 0; i < nGang; i++)
	{
		sld = characterFromID("Gang_" + i);
		LAi_SetActorType(sld);
		LAi_ActorGoToLocation(sld, "reload", sReload, "none", "", "", "", -1.0);
...
{
	int iCap2 = GetCharacterIndex("CapComission_2");
	if (iCap2 > 0)
	{
		characters[iCap2].lifeDay = 0;
	}
	DeleteAttribute(pchar, "GenQuest.CaptainComission");
}
...
void CaptainComission_ChangeCaptain()
{
	...
	chref = GetCharacter(NPC_GenerateCharacter("CapComission_2", "citiz_"+(rand(9)+51), "man", "man", iRank, iNation, -1, true)); //watch_quest_moment
	...
	chref.cirassId = FindItem("cirass5");  // предмета нет, но влияение есть
	LAi_SetHP(chref, 200 + makeint(pchar.rank) * 5, 200 + makeint(pchar.rank) * 5);
...
void CaptainComission_GangDied(string qName)
{
	if(pchar.GenQuest.CaptainComission.variant == "A1")
	{
		...
		AddQuestUserData("CaptainComission2", "sName", pchar.GenQuest.CaptainComission.Name);
		AddQuestUserData("CaptainComission2", "sShoreName", UpperFirst(XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Pre")));
		CloseQuestHeader("CaptainComission2");	
...
void CaptainComission_GenerateRumourCaptainPrison()
{
	...
	AddSimpleRumourCity(LinkRandPhrase(StringFromKey("Common_rumours_142", GetAddress_Form(pchar), sShipType, pchar.GenQuest.CaptainComission.Name),
		StringFromKey("Common_rumours_143", sShipType, pchar.GenQuest.CaptainComission.Name, GetAddress_Form(pchar)),
		RandPhraseSimple(StringFromKey("Common_rumours_144", pchar.GenQuest.CaptainComission.Name, GetAddress_Form(pchar)),
		StringFromKey("Common_rumours_145", sShipType, pchar.GenQuest.CaptainComission.Name))),
...
{
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_1" , "officer_2", "man", "man", 10, sti(pchar.GenQuest.CaptainComission.Nation), -1, true)); //watch_quest_moment; аналога ванильного officer_12 нет
...
void CaptainComission_CapEscapeTimer(string qName)
{
	...
	Log_Info(StringFromKey("InfoMessages_112"));
	ref sld = characterFromId("CapComission_1");
	RemovePassenger(pchar, sld);
	ChangeCharacterAddress(sld, "none", "");
	sld.lifeDay = 0;
	...
	SetFunctionTimerCondition("CaptainComission_CapEscapeCleanup", 0, 0, 30, false);
}

void CaptainComission_CapEscapeCleanup(string qName)
{
	if (CheckAttribute(pchar, "GenQuest.CaptainComission") && pchar.GenQuest.CaptainComission == "CapEscape")
	{
		pchar.quest.CaptainComission_ExitFromTown.over = "yes";
		CloseQuestHeader("CaptainComission2");
		DeleteAttribute(pchar, "GenQuest.CaptainComission");
		DeleteAttribute(pchar, "TownEscape");
	}
...
int CaptainComission_ApplyGoodsOverload()
{
	int g = sti(pchar.GenQuest.CaptainComission.Goods);
	int iFree = GetSquadronFreeSpace(pchar, g);
	if (iFree < sti(pchar.GenQuest.CaptainComission.GoodsQty)) pchar.GenQuest.CaptainComission.GoodsQty = iFree;
	SetCharacterGoods(pchar, g, GetCargoGoods(pchar, g) + sti(pchar.GenQuest.CaptainComission.GoodsQty));
	return iFree;
}
...
void CaptainComission_DeceivedGovernor(string qName) // лесник. если обманул губера
{
	pchar.quest.CaptainComission_DeceivedGovernor.over = "yes";
	OfficersReaction("bad");
	ChangeCharacterReputation(pchar, -50);
	if (CheckAttribute(pchar, "QuestInfo.CaptainComission2") && pchar.QuestInfo.CaptainComission2.Complete != true)
	{
		AddQuestRecord("CaptainComission2", "51");
		AddQuestUserData("CaptainComission2", "sSex", GetSexPhrase("","а"));
	}
}
...
void CaptainComission_CaptainDiedDuel(string qName)
{
	if(CheckAttribute(pchar, "GenQuest.CaptainComission.Duel"))
	{
		...
		AddQuestRecord("CaptainComission2", "53");
		AddQuestUserData("CaptainComission2", "sSex", GetSexPhrase("ся","ась"));
		AddQuestUserData("CaptainComission2", "sName", pchar.GenQuest.CaptainComission.Name);	
	}
	else
...
{
	string sShipType = GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipType),"Name") + "Acc"));
	AddSimpleRumourCity(LinkRandPhrase(StringFromKey("Common_rumours_146", GetAddress_Form(pchar), sShipType, pchar.GenQuest.CaptainComission.Name),
		StringFromKey("Common_rumours_147", GetAddress_Form(pchar), sShipType, pchar.GenQuest.CaptainComission.Name),
		RandPhraseSimple(StringFromKey("Common_rumours_148", GetAddress_Form(pchar)),
		StringFromKey("Common_rumours_149", pchar.GenQuest.CaptainComission.Name, sShipType, GetAddress_Form(pchar)))),
...
void CaptainComission_GenerateCanoneer(string qName)
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("CapComission_Canoneer", "citiz_60", "man", "man", 10, sti(pchar.GenQuest.CaptainComission.Nation), -1, true)); //watch_quest_moment; поставил с повязкой, _15 сейчас циклоп
...
void CaptainComission_NoGangDialog(string qName)
{
	...
	sld.LifeDay = 0;
	int i, iTemp, nGang = sti(pchar.GenQuest.CaptainComission.GangNum);
	for(i = 0; i < nGang; i++)
	{
		iTemp = GetCharacterIndex("Gang_" + i);
		if (iTemp != -1)
...
{
	ref sld;
	int iGoods = CaptainComission_ApplyGoodsOverload();
	...
	sld = CharacterFromId("CapComission_Canoneer"); // лесник . изменил условие . прежнее не работало при смерти канонира.
	 if (!LAi_IsDead(sld))
	{
		Log_Info(StringFromKey("InfoMessages_113")); // лесник добавил лог
		sld = CharacterFromId("CapComission_Canoneer");
		AddPassenger(pchar, sld, false);
		SetCharacterRemovable(sld, false);
    ...
    pchar.quest.CaptainComission_MapEnterAfterShoreBattle.win_condition = "CaptainComission_MapEnterAfterShoreBattle";
}
...
void CaptainComission_EnterToSeaAfterShoreBattle()
{
	...
	for (int i = 1; i <= 3; i++)
    {
		sld = GetCharacter(NPC_GenerateCharacter("Pirate_Captain0" + i, "citiz_"+(rand(9)+51), "man", "man", 5, PIRATE, 3, true)); //watch_quest_moment
...
void Convict_LocExit(string qName)
{
	ref sld;
	int i;
	if (CheckAttribute(pchar, "GenQuest.Convict.ShoreLocation"))
	...
	if(pchar.GenQuest.Convict == "FightInShore" || pchar.GenQuest.Convict == "ShoreFight" || pchar.GenQuest.Convict == "MayakFight")
	{
		if(pchar.GenQuest.Convict == "FightInShore")
		{
			...
			AddQuestUserData("Convict", "sSex", GetSexPhrase(StringFromKey("SexPhrase_23"),StringFromKey("SexPhrase_24")));
...
string ShipWreck_GetStringQty(int Qty)
{
	...
	switch(Qty)
	{
		case 3:    	strQty = XI_ConvertString("three"); break;
		case 4:    	strQty = XI_ConvertString("four"); 	break;
		case 5: 	strQty = XI_ConvertString("five"); 	break;
		case 6: 	strQty = XI_ConvertString("six");	break;
		case 7: 	strQty = XI_ConvertString("seven");	break;
		case 8: 	strQty = XI_ConvertString("eight");	break;
		case 9: 	strQty = XI_ConvertString("nine");	break;
		case 10:	strQty = XI_ConvertString("ten");	break;
	}
	return strQty;
}
...
void ShipWreck_SetCapToMap()
{
	...
	Map_CreateTrader(character.fromShore, character.toShore, "ShipWreck_BadPirate", GetMaxDaysFromColony2Colony(character.toCity, character.fromCity) + 15);
...
void ShipWreck_DeliveToCity(string qName)
{
	...
	{
		chrDisableReloadToLocation = true;
		Log_Info(StringFromKey("InfoMessages_114"));
...
void ShipWreck_afterCabinFight(string qName)
{
	...
	pchar.quest.DeadlineHasExpired.over = "yes"; // лесник сброс таймера.
}
...
void ShipWreck_ShipSink(string qName)
{
	...
	{
		AddQuestUserData("ShipWrecked", "sText", StringFromKey("SexPhrase_25"));
	}
	else
	{
		AddQuestUserData("ShipWrecked", "sText", StringFromKey("SexPhrase_26"));
	}
	// <-- belamour
	CloseQuestHeader("ShipWrecked");
	DeleteAttribute(pchar, "GenQuest.ShipWreck");	
...
void Hold_GenQuest_Init(ref chref)
{
	...
	switch(sti(chref.Hold_GenQuest.variant))	
	{
		case 0: // "наводка"
			...
			sColony = Sea_FindNearColony(true);
			if (sColony != "none") 
			{
...
void Hold_GenQuest_SetMerchant(string qName)
{
	...
	int IslandIdx = FindIsland(GetArealByCityName(pchar.GenQuest.Hold_GenQuest.City));
	ref pLoc = &Islands[IslandIdx];
	float x, y, z;
	FindIslandLocatorXYZ(IslandIdx, "reload1", &x, &y, &z);
	string sLocator = GetSeaQuestShipFarLocator(pLoc, "Quest_ships", x, y, z);
...
void Hold_GenQuest_GenerateChar()
{
	...
	ref sld = GetCharacter(NPC_GenerateCharacter("Hold_GenQuestChar", "trader_"+(rand(15)+1), "man", "man", 10, sti(pchar.GenQuest.Hold_GenQuest.Nation), 3, true)); //партнёр торгаша
...
{
	ref sld = GetCharacter(NPC_GenerateCharacter("Hold_GenQuestChar", "citiz_"+(rand(9)+41), "man", "man", 5, sti(pchar.GenQuest.Hold_GenQuest.Nation), 3, true)); //бывший бандит
	sld.name = pchar.GenQuest.Hold_GenQuest.Name;
	sld.lastname = "";
	LAi_SetImmortal(sld, true);
...
//=====================================================================================================================================
// Генератор "Правосудие на продажу" (Warship, Rosarak) -->
//=====================================================================================================================================

// Выход из локации получения квеста
void JusticeOnSale_LocationExit(string _quest)
{
	DoQuestCheckDelay("DeleteGroupOnExitLocation", 0.0);
...
// Просрочил по времени
void JusticeOnSale_Over(string _quest)
{
	DeleteQuestCondition("JusticeOnSale_ShoreEnterWithSmuggler");
	DeleteQuestCondition("JusticeOnSale_ShoreEnterFromMayor");
	
	if(CheckAttribute(PChar, "GenQuest.JusticeOnSale.MayorQuest")) //Был на стороне мэра
	{
		AddQuestRecord("JusticeOnSale", "8");
		ref sld = CharacterFromID(PChar.GenQuest.JusticeOnSale.CityId + "_Mayor");
		IncreaseNPCDispositionEx(sld, pchar, false, -5);
		ChangeCharacterReputation(pchar, -4);
		ChangeCharacterNationReputation(pchar, sti(sld.nation), -1);
	}
	else
	{
		if(!CheckAttribute(PChar, "GenQuest.JusticeOnSale.CaptainFreed")) //Ничего не сделал (капитан не освобождён)
		{
			AddQuestRecord("JusticeOnSale", "8_1");
			AddQuestUserData("JusticeOnSale", "sSex", GetSexPhrase("","а"));
		}
		else //Освободил, но не пошёл в бухту за наградой
		{
			AddQuestRecord("JusticeOnSale", "8_2");
			AddQuestUserData("JusticeOnSale", "sSex", GetSexPhrase("","а"));
			AddQuestUserData("JusticeOnSale", "sSex2", GetSexPhrase("ся","ась"));
		}
	}
...
// Освобождение убеждением
void JusticeOnSale_CaptainPeace() //_quest не нужен тут
{
	PChar.GenQuest.JusticeOnSale.CaptainFreed = true; // > капитан реально освобождён (для корректных записей 3_2 / 8_2)
	AddCharacterExpToSkill(pchar, "Leadership", 50);
	ChangeContrabandRelation(pchar, 20);
	ref sld = CharacterFromID("JusticeOnSale_Smuggler");
	sld.LifeDay = 0; //Потом всё равно будет его клон
	LAi_SetImmortal(sld, true);
	ChangeCharacterAddressGroup(sld, loadedlocation.id, "reload", "reload12");
	LAi_SetActorTypeNoGroup(sld);
	LAi_ActorGoToLocation(sld, "reload", "reload1", "none", "", "", "", -1);
	JusticeOnSale_DeleteShip();
	DeleteQuestCondition("JusticeOnSale_CaptainDeath");
	
	AddQuestRecord("JusticeOnSale", "3_4");
	AddQuestUserData("JusticeOnSale", "sSex1", GetSexPhrase("ся","ась"));
	AddQuestUserData("JusticeOnSale", "sSex2", GetSexPhrase("","а"));
	AddQuestUserData("JusticeOnSale", "Shore", XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Acc"));
...
// Вызволение с боем
void JusticeOnSale_CaptainFight() //_quest не нужен тут
{
	PlaySound("Key");
	int iNation = sti(PChar.GenQuest.JusticeOnSale.Nation);
	ChangeCharacterReputation(pchar, -5);	//минус в репу, коль решил устроить резню
	ChangeCharacterNationReputation(pchar, iNation, -2);
	ref sld = CharacterFromID("JusticeOnSale_Smuggler");
	GiveItem2Character(sld, "unarmed");		//Обязательно выдаём, хоть он и не будет драться
	EquipCharacterByItem(sld, "unarmed");	//Иначе враги будут игнорировать
	ChangeCharacterAddressGroup(sld, loadedlocation.id, "reload", "reload12");
	LAi_SetActorTypeNoGroup(sld);
	LAi_ActorFollowEverywhere(sld, "", -1);
	LAi_group_MoveCharacter(sld, LAI_GROUP_PLAYER);
	...
	string sGroup = GetNationNameByType(iNation) + "_citizens";
	...
	for(int i = 0; i < LAi_numloginedcharacters; i++)
	{
		...
		if(idx < 0) continue;
		sld = &characters[idx];
		if(sld.chr_ai.group == sGroup)
		{
			if(sld.chr_ai.type == "guardian")
			{	//солдатня бежит, иначе это совсем простой квест
				LAi_tmpl_runto_InitTemplate(sld);
				LAi_tmpl_runto_SetLocator(sld, "reload", "reload12", -1.0);
			}
			else
			{	//и начальник с ними
				LAi_type_huber_prefight(sld);
	...
	SetFunctionExitFromLocationCondition("JusticeOnSale_CaptainEscape", pchar.location, false);
}

// Беготня в городе
void JusticeOnSale_CaptainEscape(string _quest)
{
	PChar.GenQuest.JusticeOnSale.Escape = true;
	SetFunctionExitFromLocationCondition("JusticeOnSale_CaptainEscape_Fail", pchar.location, false);

	ref sld = CharacterFromID("JusticeOnSale_Smuggler");
	ChangeCharacterAddressGroupEx(sld, loadedlocation.id, pchar.location.group, pchar.location.locator, true);
	LAi_ActorRunToLocation(sld, "reload", "gate_back", "none", "", "", "JusticeOnSale_CaptainEscape_Done", -1.0);
}

void JusticeOnSale_CaptainEscape_Fail(string _quest)
{
	AddQuestRecord("JusticeOnSale", "3_3");
	AddQuestUserData("JusticeOnSale", "sSex", GetSexPhrase("ому","ой"));
	JusticeOnSale_QuestEnd();
}

// Стереть капитана
void JusticeOnSale_DeleteCaptain()
{
	DeleteQuestCondition("JusticeOnSale_CaptainDeath");
	if (GetCharacterIndex("JusticeOnSale_Smuggler") != -1)
	{
		ref sld = CharacterFromID("JusticeOnSale_Smuggler");
		ChangeCharacterAddressGroup(sld, "none", "", "");
		sld.LifeDay = 0;
...
// Стереть корабль
void JusticeOnSale_DeleteShip()
{
	DeleteQuestCondition("JusticeOnSale_PirateShip_Sink");
	DeleteQuestCondition("JusticeOnSale_PirateShip_Boarding");
	if (Group_FindGroup("JusticeOnSaleGroup") != -1)
		Group_DeleteGroup("JusticeOnSaleGroup");
	if (GetCharacterIndex("JusticeOnSale_ShipPirate") != -1)
	{
		ref sld = CharacterFromID("JusticeOnSale_ShipPirate");
		sld.LifeDay = 0;
	}
}

// Смерть капитана
void JusticeOnSale_CaptainDeath(string _quest)
{
	//ChangeContrabandRelation(pchar, -5);
	DeleteQuestCondition("JusticeOnSale_Over");
	DeleteQuestCondition("JusticeOnSale_ShoreEnterWithSmuggler");
	
	if(!CheckAttribute(Pchar, "GenQuest.JusticeOnSale.CaptainFreed"))
	{	//в тюрьме (капитан не освобождён)
		AddQuestRecord("JusticeOnSale", "3_1");
		AddQuestUserData("JusticeOnSale", "sName", PChar.GenQuest.JusticeOnSale.SmugglerName);
	}
	else
	{	//в городе (капитан уже освобождён)
		AddQuestRecord("JusticeOnSale", "3_2");
	}
...
void JusticeOnSale_ShoreEnterWithSmuggler(string _quest)
{
	...
	arrayNPCModelHow = 0;
	for(int i = 0; i < sti(PChar.GenQuest.JusticeOnSale.EncQty); i++)
	{
		character = GetCharacter(NPC_GenerateCharacter("JusticeOnSale_ShorePirate_" + i, "citiz_"+(rand(9)+21), "man", "man", sti(PChar.rank), PIRATE, -1, true));
		SetNPCModelUniq(character, "smuggler", MAN);
		...
		if(i == 0)
		{	//Клон
			character.model	= PChar.GenQuest.JusticeOnSale.SmugglerModel;
			character.name	= PChar.GenQuest.JusticeOnSale.SmugglerName;
			character.lastName = "";
			character.greeting = "marginal";
			Characters_RefreshModel(character);
			LAi_ActorDialog(character, PChar, "", 3, 3);
		}
		else LAi_ActorFollow(character, CharacterFromID("JusticeOnSale_ShorePirate_0"), "", -1);
	}

	chrDisableReloadToLocation = true;
	
	DeleteAttribute(location, "DisableEncounters");
}
...
void JusticeOnSale_ShoreEnterFromMayor(string _quest)
{
	...
	for(int i = 0; i < 4 ; i++)
	{
		character = GetCharacter(NPC_GenerateCharacter("JusticeOnSale_ShorePirate_" + i, "citiz_"+(rand(9)+21), "man", "man", sti(PChar.rank), PIRATE, 0, true));
		SetNPCModelUniq(character, "smuggler", MAN);
...
// Выход из локации бухты
void JusticeOnSale_LocExitAfterMayor(string _quest)
{
	Delay_DeleteGroup("JusticeOnSale_ShoreGroup");
	JusticeOnSale_ChangeContraRep();
	
	if(!CheckAttribute(PChar, "GenQuest.JusticeOnSale.KillSmugglers"))
	{
		AddQuestRecord("JusticeOnSale", "6_2");
		AddQuestUserData("JusticeOnSale", "sSex", GetSexPhrase("","ла"));
		ref sld = CharacterFromID(PChar.GenQuest.JusticeOnSale.CityId + "_Mayor");
		IncreaseNPCDispositionEx(sld, pchar, false, -3);
		ChangeCharacterReputation(pchar, -2);
		ChangeCharacterNationReputation(pchar, sti(sld.nation), -1);
		JusticeOnSale_QuestEnd();
		return;
	...
	DoQuestFunctionDelay("JusticeOnSale_SeaCheck", 0.0); //т.к. bSeaActive обновляется позднее
}

void JusticeOnSale_SeaCheck(string _quest)
{
	if(!bSeaActive)
	{	//Вышел не в море
		ref sld = CharacterFromID(PChar.GenQuest.JusticeOnSale.CityId + "_Mayor");
		...
		ChangeCharacterReputation(pchar, -2);
		ChangeCharacterNationReputation(pchar, sti(sld.nation), -1);
		AddQuestRecord("JusticeOnSale", "6_1");
		AddQuestUserData("JusticeOnSale", "sSex", GetSexPhrase("","а"));
		JusticeOnSale_QuestEnd();
	}
}
...
void JusticeOnSale_PirateShip_Sink(string _quest)
{
	...
	{	// Сделали все правильно
		sld.JusticeOnSalePrize = true;
		AddQuestRecord("JusticeOnSale", "7");
		JusticeOnSale_QuestEnd();
		ReOpenQuestHeader("JusticeOnSale"); //закроется в диалоге с мэром
	}
	else
	{	// Не убив контриков в бухте, сразу напали на корабль
...
// Уплыли, не потопив корабль
void JusticeOnSale_ExitFromSea(string _quest)
{
	ref sld = CharacterFromID(PChar.GenQuest.JusticeOnSale.CityId + "_Mayor");
	IncreaseNPCDispositionEx(sld, pchar, false, -3);
	ChangeCharacterReputation(pchar, -2);
	ChangeCharacterNationReputation(pchar, sti(sld.nation), -1);
	sld = CharacterFromID("JusticeOnSale_ShipPirate");
	AddQuestRecord("JusticeOnSale", "6_3");
	AddQuestUserData("JusticeOnSale", "sShip", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName + "Ins")));
	JusticeOnSale_QuestEnd();
}
...
void JusticeOnSale_KillSmugglers(string _quest)
{
	...
	if(GetCharacterShipLocation(PChar) == PChar.GenQuest.JusticeOnSale.ShoreId)
	{	// Сделали все правильно
		Delay_DeleteGroup("JusticeOnSale_ShoreGroup");
		sld = CharacterFromID("JusticeOnSale_ShipPirate");
	...
	{	// Неверный алгоритм
		AddQuestRecord("JusticeOnSale", "6_1");
		AddQuestUserData("JusticeOnSale", "sSex", GetSexPhrase("","а"));
		sld = CharacterFromID(PChar.GenQuest.JusticeOnSale.CityId + "_Mayor");
		IncreaseNPCDispositionEx(sld, pchar, false, -3);
		ChangeCharacterReputation(pchar, -2);
		ChangeCharacterNationReputation(pchar, sti(sld.nation), -1);
		JusticeOnSale_QuestEnd();
...
// Изменение репутации должно происходить 1 раз
// Когда контра узнаёт, что именно ГГ поручено их уничтожить
void JusticeOnSale_ChangeContraRep()
{	
	if(CheckAttribute(pchar, "GenQuest.JusticeOnSale"))
	{
		if(!CheckAttribute(pchar, "GenQuest.JusticeOnSale.ContraRep"))
		{	//с учётом того, что ещё -10 было при доносе
			ChangeContrabandRelation(pchar, -25);
		}
		pchar.GenQuest.JusticeOnSale.ContraRep = true;
	}
}

// > Правосудие на продажу - результат проверки убеждения
string JusticeOnSale_PersuadeTarget()
{
	bool bOk = GetCharacterSkillToOld(Pchar, "Fencing") >= 7 && sti(Pchar.Rank) >= 10 && sti(PChar.reputation) <= 30;
	if (bOk || CheckCharacterPerk(pchar, "SeaDogProfessional"))
		return "JusticeOnSale_5";
	int iVal = makeint((
	...
	if (iVal <= rand(35))
		return "JusticeOnSale_5";
	return "JusticeOnSale_5_1";
}

// Конец квеста, затрём всё
void JusticeOnSale_QuestEnd()
{
	aref quests;
	string sTemp;
	makearef(quests, pchar.quest);
	int n = GetAttributesNum(quests);
	for(int i = 0; i < n; i++)
	{
		sTemp = GetAttributeName(GetAttributeN(quests,i));
		if(HasStr(sTemp,"JusticeOnSale"))
			pchar.quest.(sTemp).over = "yes";
	}
	JusticeOnSale_DeleteShip();
	JusticeOnSale_DeleteCaptain();
	// > подчистить NPC в бухте (мирный/побег путь)
	int iEnc = sti(PChar.GenQuest.JusticeOnSale.EncQty);
	ref sp;
	for(int j = 0; j < iEnc; j++)
	{
		if(GetCharacterIndex("JusticeOnSale_ShorePirate_" + j) != -1)
		{
			sp = CharacterFromID("JusticeOnSale_ShorePirate_" + j);
			ChangeCharacterAddressGroup(sp, "none", "", "");
			sp.LifeDay = 0;
		}
	}
...
//=====================================================================================================================================
void NormalizeRoutedCitizen(string qName)
{
    if (CheckAttribute(&TEV, "CitizenHelpRoutes.follower"))
    {
        ref sld = characterFromId(TEV.CitizenHelpRoutes.follower);
