...
void InitPsHeros()
{
	...
	for (n=1; n<=heroQty; n++)
	{
        if (n != startHeroType) //токо не ГГ
        {
			...
			//navy <--
			SetFantomParamFromRank(ch, 1 + rand(5), true); //генерим статы TODO на отдельно специал и эмулятор скилов
			trace("PGG " + GetFullName(ch) + " starting rank " + sti(ch.rank));
			ch.perks.list.ByWorker = "1";
			ch.perks.list.ShipEscape = "1";
			ch.perks.list.Energaiser = "1";
			ch.loyality = 10 + rand(10); //лояльность
			if (sti(ch.reputation) > 41)
			{
				ch.alignment = "good";
				SetCharacterPerk(ch, "Honest");
				
			}
			else
			{
				ch.alignment = "bad";
				SetCharacterPerk(ch, "Rebel");
			}
			SetCharacterPerk(ch, "PGGTrait");
			//TODO: некоторым можно поставить черту на роман, но пока не будем
...
//	i = GetEventData();
	...
	if (IsCompanion(chr))
	{
		...
		{
			//тут возможно стоит дописать ещё и общую лояльность и мораль матросов.
...
//--------------------------------------------------
	if (rand(100) < 5)
	{
		...
		//трем сабли.
		sWeapon = FindCharacterItemByGroup(chr, BLADE_ITEM_TYPE);
		while(sWeapon != "")
		{
			TakeItemFromCharacter(chr, sWeapon);
			sWeapon = FindCharacterItemByGroup(chr, BLADE_ITEM_TYPE);
		}

		ItemTakeEquip(chr, LAi_NPC_EquipBladeSelection(chr), 0);
		...
		{
			//трем пистолеты.
			sWeapon = FindCharacterItemByGroup(chr, GUN_ITEM_TYPE);
			while(sWeapon != "")
			{
				TakeItemFromCharacter(chr, sWeapon);
				sWeapon = FindCharacterItemByGroup(chr, GUN_ITEM_TYPE);
			...
			if(rand(100) < 50)
			{
				ItemTakeEquip(chr, "pistol1", 0);
			}
			else
			{
				if(rand(100) < 50)
				{
					ItemTakeEquip(chr, "pistol2", 0);
				}
				else
				{
					if(rand(100) < 50)
					{
						ItemTakeEquip(chr, "pistol5", 0);
					}
					else
					{
						if(rand(100) < 70)
    					{
							ItemTakeEquip(chr, "pistol3", 0);
    					}
						else
						{
							if (rand(100) < 80)
        					{
								ItemTakeEquip(chr, "pistol6", 0);
        					}
							else
							{
								if (sti(chr.rank) > 9)
								{
									ItemTakeEquip(chr, "pistol4", 0);
        						}
        					}
    					}
					}
			...
			if (sWeapon != "")
			{
				...
				{
					if (HasStrEx(sCharge, "grapeshot,GunEchin", "&"))
					{
						switch (rand(2))
						{
							case 0: sAmmo = "grapeshot"; break;
							case 1: sAmmo = "GunEchin"; break;
							case 2: sAmmo = "grapeshot"; break;
						}
						
						if (HasStr(sCharge, "harpoon") && rand(2) == 0)
							sAmmo = "harpoon";
					}
					else
					{
						if (HasStr(sCharge, "_colt"))
						{
							sAmmo = "bullet_colt";
							
							if (rand(2) == 0)
								sAmmo = "cartridge_colt";
						}
						else
						{
							if (HasStrEx(sCharge, "grenade,petard", "&"))
							{
								sAmmo = "grenade";
				...
				LAi_SetCharacterUseBullet(chr, GUN_ITEM_TYPE, sAmmo);
				
				if (GetCharacterItem(chr, sAmmo) < 10)
					TakeNItems(chr, sAmmo, 5+rand(15));
				
				sAmmo = LAi_GetAmmoGunpowderType(sAmmo);
				
				if (sAmmo != "" && GetCharacterItem(chr, sAmmo) < 10)
					AddItems(chr, sAmmo, 5+rand(15));
			}
		}
	}
}
...
void PGG_TavernCheckIsPGGHere()
{
	...
	{
		chr = CharacterFromID("PsHero_" + i);
		if (HasSubStr(pchar.location, chr.PGGAi.location.town) && !LAi_IsDead(chr) && chr.PGGAi.location != "Dead") //закрыл дополнительно.
		{
...
void PGG_PlaceCharacter2Tavern(ref chr, bool _bSet)
{
	...
	if (_bSet)
	{
		...
		n = 0;
		while (HasSubStr(sTemp, "sit_front") && n < 50)
...
{
	IncreaseNPCDispositionEx(pgg, chr, false, _addValue*2);
	PGG_DebugLog(pgg.id + " changed relation to " + chr.id + " " + _addValue);
	return ConvertPGGRelation(GetNPCDispositionEx(pgg, chr), false);
}
...
void PGG_SetRelation2OtherAsNormal(ref pgg)
{
	...
	{
		chr = CharacterFromID("PsHero_" + i);
		if (chr.id != pgg.id) IncreaseNPCDispositionEx(pgg, chr, false, 0);
	}
	IncreaseNPCDisposition(pgg, 0);
}

//Rosarak биекция для обратной совместимости
int ConvertPGGRelation(int iRel, bool to_disposition)
{
	if(to_disposition) return iRel*2-100; //old pgg relation to disposition
	else return (iRel+100)/2; // disposition to old pgg relation
}
...
void PGG_OnInit_SailToIsland(ref rTmpl)
{
	...
	oPrm.PsHero_Name = GetFullName(chr);
	oPrm.To = XI_ConvertString("In");
	if (chr.PGGAi.Task.Target == "Tortuga") oPrm.To = XI_ConvertString("OnShip");
...
//		LAi_warrior_SetStay(chr, true); //можно убрать, будет шататься по бухте.
		...
		for (i = 0; i < iTmp; i++)
		{
			sld = GetCharacter(NPC_GenerateCharacter("Rand_Enemy0" + i, "pirate_"+(rand(15)+1), "man", "man", 5, PIRATE, 3, true));
...
void PGG_BecomeHiredOfficer()
{
	...
	chr.location = "None";
	chr.PGGAi.location.town = XI_ConvertString("Officer");
	chr.PGGAi.IsPGG = false;
	if (chr.sex == "woman") chr.Dialog.FileName = "Officer_Girl.c";
	else chr.Dialog.FileName = "Officer_Man.c";
...
void PGG_FireOfficer()
{
	...
	{ //HardCoffee фикс для увольнения в каюте (чтобы ушёл)
		if (CheckAttribute(loadedLocation, "locators.reload.reload1"))
		{
			...
			LAi_ActorGoToLocation(chr, "reload", "reload1", "none", "", "", "", 4.0);
		}
		else ChangeCharacterAddress(chr, "none", "");
	}
	else chr.location = "none";
}
...
bool PGG_CheckForQuestOffer(ref chr)
{
	...
	if (bDisableLandEncounters) return retVal; //если квестовый разговор, то нечего над душой стоять
	...
	if (bBettaTestMode && CheckAttribute(PChar, "PGGAlwaysQuest")) iTst = 10; //в версии нет этого атрибута
...
void PGG_Q1PlaceShipsNearIsland()
{
	...
	for (i = 0; i < iRnd; i++)
	{
		...
		if (i < 2)
		{
			chr.Ship.Type = GenerateShipExt(SHIP_PINNACE + rand(SHIP_LINESHIP - SHIP_PINNACE), 1, chr);
			SetBaseShipData(chr);
			SetCrewQuantityFull(chr);
...
void PGG_Q1LocationLoaded(string qName)
{
	...
	for (i = 1; i < iRnd; i++)
	{
		...
		chr = GetCharacter(NPC_GenerateCharacter("PGGSailor_"+i, attrName, "man", "man", 15, PIRATE, 0, true));
...
void PGG_Q1FightOnShore()
{
	...
	if (!CheckAttribute(PChar, "GenQuest.PGG_Quest.PGGid.Dead"))
	{
		...
		for (i = 1; i < iRnd; i++)
		{
			SetFantomDefenceLocal("goto", "", PIRATE, LAI_GROUP_PLAYER);
		}
	...
	for (i = 1; i < iRnd; i++)
	{
		chr = SetFantomDefenceLocal("officers", sLoc, iNation, "PGGTmp");
		chr.id = "pirate_" + i;
	}
	//натравим.
