...
void LAi_tmpl_dialog_CharacterUpdate(aref chr, float dltTime)
{
	...
	// Временная затычка -->
	bool tFix = loadedLocation.filespath.models == "locations\inside\tavern02" && chr.location.locator == "sit_base2" && Characters[idx].location.locator == "sit_front1";
	// <-- Временная затычка; TODO: заменить, например, после перестановки стула к центру стола
	...
	{
		if (CheckAttribute(chr, "nonTable")) return; //режем диалоговые анимации сидунам не за столом
		time = stf(chr.chr_ai.tmpl.phrasetime) - dltTime;
		if(time < 0.0)
		{			
