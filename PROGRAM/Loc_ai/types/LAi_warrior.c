...
//	if (!CheckAttribute(chr, "chr_ai.type.dialog")) //TODO убрать как появится причина появления ошибки RUNTIME ERROR in Loc_ai\types\LAi_warrior.c(64): missed attribute: model
//	{
//	    trace("====================================================================================");
//	    DumpAttributes(chr);
//	    trace("====================================================================================");
...
void LAi_type_warrior_CharacterUpdate(aref chr, float dltTime)
{
	...
	else
	{
		...
		else
		{
			...
			{
				//Стоим и ждём цели
				if (!CheckAttribute(chr, "stay_man")) LAi_type_warrior_SetWateState(chr);
			}
			if (CheckAttribute(chr, "time_to_go") && sti(chr.time_to_go) < 250) chr.time_to_go = sti(chr.time_to_go) + 1;
			//слежение залезания ГГ в боксы
			if (CheckAttribute(chr, "watchBoxes") && chr.chr_ai.tmpl != LAI_TMPL_DIALOG && !CheckAttribute(loadedlocation, "freeboxes")) 
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					{					
						...
						else if (chr.location == "SanAugustineResidence" && pchar.chr_ai.tmpl != LAI_TMPL_DIALOG && !CheckAttribute(chr, "HiredDate") && chr.id != "Ment_6")
						{
                            ...
                            if (locy < 8.5)
                            {
                                if (!CheckAttribute(chr, "time_to_go") || sti(chr.time_to_go) >= 250)
                                {
                                    float x, y, z;
                                    ...
                                    if (dist < 10.0 && LAi_Character_CanDialog(chr, pchar))
                                    {
                                        chr.dialog.currentnode = "GetOutNow";
...
{	
	...
	if(chr.chr_ai.tmpl == LAI_TMPL_CABIN) return true; //HardCoffee хождение по каюте
	return false;
}
...
void LAi_type_warrior_SetWateState(aref chr)
{
	if(sti(chr.chr_ai.type.stay) == 0)
	{
		if(chr.chr_ai.type.index != "")
		{
			...
			{
				if (Get_My_Cabin() == chr.location) LAi_tmpl_cabin_InitTemplate(chr); //HardCoffee хождение по каюте
				//Гуляем в поисках цели
				else LAi_tmpl_walk_InitTemplate(chr);
			}
		}
		else
		{
...
void LAi_type_warrior_questPanama(ref chr)
{
	...
	{
		Log_SetStringToLog(StringFromKey("InfoMessages_67"));
		int iTemp;
	...
	RemoveLandQuestmark_Main(chr, "Pir_Line");
}
