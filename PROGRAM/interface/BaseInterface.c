...
{
	questTextFileName[0] = "RESOURCE\INI\texts\" + LanguageGetLanguage() + "\QuestBook\QuestBook_Part1.txt";
...
void InitInterfaceTables()
{
	...
	Interfaces[INTERFACE_PORTMAN].SectionName = "interface\portman.c";
	Interfaces[INTERFACE_PORTMAN].IniFile = "RESOURCE\INI\INTERFACES\portman.ini";
	...
	// интерфейс торговли с контрабандистами
	Interfaces[INTERFACE_CONTRABAND].SectionName = "interface\contraband.c";
	Interfaces[INTERFACE_CONTRABAND].IniFile = "RESOURCE\INI\INTERFACES\contraband.ini";

	// интерфейс починки в бухте
	...
	// Интерфейс информационного сообщения
	Interfaces[INTERFACE_INFOMESSAGE].SectionName = "interface\infoMessage.c";
	Interfaces[INTERFACE_INFOMESSAGE].IniFile     = "RESOURCE\INI\INTERFACES\infoMessage.ini";
	
	// evganat - энциклопедия
	Interfaces[INTERFACE_ENCYCLOPEDIA].SectionName = "interface\Encyclopedia.c";
	Interfaces[INTERFACE_ENCYCLOPEDIA].IniFile = "RESOURCE\INI\INTERFACES\Encyclopedia.ini";
	
	// AlexBlade - изменение внешнего вида корабля на верфи
	Interfaces[INTERFACE_SHIP_CUSTOMIZE].SectionName = "interface\ShipCustomize.c";
	Interfaces[INTERFACE_SHIP_CUSTOMIZE].IniFile = "RESOURCE\INI\INTERFACES\ShipCustomize.ini";
