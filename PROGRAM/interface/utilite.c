...
native int StringCompare(string strA, string strB); // 1 - strA > strB, -1 - strA < strB, 0 - strA == strB
native string StringToUpper(string sBase);
...
string FloatToString(float fl,int nDigAfterPoint)
{
  return fts(fl, nDigAfterPoint); // TODO Плагин показыает ошибку, но функция рабочая из C
}
		...
		for (n = 0; n < q; n++)
		{
			...
			if (cn >= 0)
			{
				if (!CheckAttribute(&characters[cn], "prisoned") || sti(characters[cn].prisoned) != true)
				{
					...
					{
						if (!CheckAttribute(&characters[cn], "isquest")) // TODO
						{
							AddFaceGroup(strAccess, "FACE128_" + Characters[cn].faceID);
						}
					}
...
void FillItemsPicturesList(string strAccess, ref chref)  // TODO del
{
...
string GetMoralePicture(float fMoraleValue)
{
	...
	{
		return "interfaces\flags\flag_1_64.tga";
	}
	if (fMoraleValue >= 50.0)
	{
		return "interfaces\flags\flag_2_64.tga";
	}
	if (fMoraleValue >= 25.0)
	{
		return "interfaces\flags\flag_3_64.tga";
	}
...
string GetExperiencePicture(float fExpValue)
{
	if (fExpValue >= 75.0)
	{
		return "interfaces\stars\Yellow_star1.tga";
	}
	if (fExpValue >= 50.0)
	{
		return "interfaces\stars\Yellow_star2.tga";
	}
	if (fExpValue >= 25.0)
	{
		return "interfaces\stars\Yellow_star3.tga";
	}
...
void SaveSavedOptionsByFile(aref gopt, string fileName)
{
	SendMessage(&GameInterface, "lsa", MSG_INTERFACE_SAVEOPTIONS, fileName, gopt);
}

void ReadSavedOptionsByFile(aref gopt, string fileName) {
...
void SaveStartGameParam()
{
	...
	if (CheckAttribute(&GameInterface, "PROFILE_NAME.str"))
	{
		optref.StartGameParam.PlayerProfile = GameInterface.PROFILE_NAME.str;
	}
	else if (CheckAttribute(&PlayerProfile, "name"))
	{
	...
	optref.StartGameParam.bWildCaribbean = bWildCaribbean;
...
void LoadStartGameParam()
{
	...
	if (CheckAttribute(optref, "StartGameParam.bWildCaribbean"))
	{
		bWildCaribbean = sti(optref.StartGameParam.bWildCaribbean);
	}
...
string GetItemDescribe(int iGoodIndex)
{
	...
	{
		if (GetAttributeQty(arItm, "potion", "health,energy,antidote,drunk") > 1)
			effectStr = LanguageConvertString(lngFileID, "Potion parameters");
		else
			effectStr = LanguageConvertString(lngFileID, "Potion parameter");
		describeStr += "\n\n" + effectStr;
		...
		{
			describeStr += " " + ColorText(LanguageConvertString(lngFileID, "health value"), "contra");
		...
		if (CheckAttribute(arItm, "potion.energy") || arItm.id == "potionrum")
		{
		...
		if (CheckAttribute(arItm, "potion.antidote") && sti(arItm.potion.antidote) > 0)
			describeStr += " " + ColorText(LanguageConvertString(lngFileID, "antidote"), "export");

		if (CheckAttribute(arItm, "potion.drunk") && stf(arItm.potion.drunk) > 0.0)
			describeStr += " " + ColorText(LanguageConvertString(lngFileID, "drunk"), "darkgray");
	}
...
// --> Сортировка перенесена из мода Качество Пиратской Жизни [1.4.0] для Caribbean Legend под авторством Pvt. Joker
// AlexBlade - CompareStringsByABC переведён на native метод
//
	...
	for (i = 0; i < tableSize - 1; i++)
	{
		for (j = 0; j < tableSize - 1 - i; j++)
		{
			...
			switch (datatype)
			{
				...
				break;
				case "string":
					if (!CheckAttribute(&upRow, cellName + ".str")) continue;
					if (!CheckAttribute(&downRow, cellName + ".str")) continue;
					lLetter = GetStrSmallRegister(upRow.(cellName).str);
					rLetter = GetStrSmallRegister(downRow.(cellName).str);
					compare = !CompareStringsByABC(lLetter, rLetter);
				break;
				...
				break;
				case "float":
					if (upRow.(cellName).str == "-") compare = stf(skipVal) > stf(downRow.(cellName).str);
					else if (downRow.(cellName).str == "-") compare = stf(upRow.(cellName).str) > stf(skipVal);
					else compare = stf(upRow.(cellName).str) > stf(downRow.(cellName).str);
				break;
				case "date":
...
// Оригинальный метод сортировки по дате от разработчика игры mitrokosta
...
bool CompareStringsByABC(string left, string right)
{
	int res = StringCompare(left, right);
	if (res < 0)
		return 1;
	if (res > 0)
		return 0;
