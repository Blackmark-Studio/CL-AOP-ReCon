...
bool ProcessCondition(aref condition)
{
	...
	if(CheckAttribute(condition,"character"))
	{
		...
		else condition.characterIdx = sti(condition.character); // делаем вывод, что это число; для строк будет 0, то есть NullCharacter
		// совместимость с кодом акеллы, где в character хранят не ID (строка), а IDX (число) <--
	}
	...
	switch(sConditionName)
	{
    	...
    	case "NPC_Death":
			if(CheckAttribute(condition, "character") && GetCharacterIndex(condition.character) == -1) return true; //Rosarak fix
    		return CharacterIsDead(refCharacter);
    	break;
    	...
    	case "item":  // TODO  пока в квесте воровства  есть, но не используется
