...
#define WDM_QUESTMARK_MAJOR "QM_major"
#define WDM_QUESTMARK_MINOR "QM_minor"
...
//если число меньше 10, то ставим 0 вначале
string FormedNumAsDate(int iNum)
{
	if (iNum < 10) return "0" + iNum;
	return "" + iNum;
}

void UpdateQuestOrder(ref rQuest)
{
    if (!CheckAttribute(&TEV, "QuestOrder")) TEV.QuestOrder = 1;//TODO обратная совместимость, потом уброать
    TEV.QuestOrder = sti(TEV.QuestOrder) + 1;
    rQuest.Order = TEV.QuestOrder;
}
...
void AddQuestRecordEx(string idQuest,string idReferenceQuest,string idText)
{
	if(CheckAttribute(pchar,"QuestInfo."+idQuest)==false)
	{
		...
		// туториал
		DoQuestFunctionDelay("Tutorial_Logbook", 1.5);
	}
	string sTextLbl = "l0";
	aref arTmp;
	//trace("Second Check");
	...
	{
		makearef(arTmp, pchar.QuestInfo.(idQuest));
		UpdateQuestOrder(&arTmp);
...
void CloseQuestHeader(string idQuest)
{
	...
	aref questRef;
	makearef(questRef, pchar.QuestInfo.(idQuest));
	//если квест уже закрыт, то дату закрытия не меняем, иначе проставляем
	if (CheckAttribute(questRef, "Complete") && questRef.Complete == false) UpdateQuestOrder(&questRef);
...
ref CharacterFromID(string characterID)
{
	...
	{
		Log_TestInfo("Error: Не найдёт НПС с characterID = "+characterID);
	}
    if (i>=0)
    {
        return &characters[i];
...
//проверить не занят ли локатор какой-нибудь вещицей
bool CheckFreeItemLocator(string sLocationName, string sLocator)
{
...
bool CheckCurLocator(string group,string locator,float x,float y,float z)
{
	...
	if(CheckAttribute(&TEV, "sitting") && group == "sit") rad *= 1.5; //Огрубляем для нормальной проверки в sit
...
//navy --> отложеный вызов квестовой функции
			...
			{
			    sFunction = pchar.PostEventQuest.questDelay.(stmp).function;
			}
			DeleteAttribute(pchar,"PostEventQuest.questDelay."+stmp);
...
//Получить неполное имя персонажа
string GetFirstName(ref chref)
{
	string retStr = "";
	if (CheckAttribute(chref,"name") && chref.name != "")
	retStr = chref.name;
	return retStr;
}
string GetLastName(ref chref)
{
	string retStr = "";
	if (CheckAttribute(chref,"lastname") && chref.lastname != "")
	retStr = chref.lastname;
	return retStr;
}
...
// boal <--
string GetFullNameTitulForm(ref chref) //манипуляции с титулом для интерфейсов
{
	string retStr = "";

	if(CheckAttribute(chref,"name"))
	{
		if(HasSubStr(chref.name, GlobalStringConvert("BaronTitle")))				retStr = FindStringAfterChar(chref.name, " ");
		else if(HasSubStr(chref.name, GlobalStringConvert("Address_Form_Eng") + " "))       retStr = UpperFirst(chref.name);
		else if(HasSubStr(chref.name, GlobalStringConvert("Address_Form_EngWoman") + " "))  retStr = UpperFirst(chref.name);
		else retStr = chref.name;
	}
	if(CheckAttribute(chref,"middlename"))
	{
	    if (chref.middlename != "") retStr = retStr + " " + chref.middlename;
...
//-------------------------------------------------------------------------------------
// TODO метод опасен, применять осторожно, не трет компаньонов, обязательно проверить работу и вернуть всех обратно RestorePassengers, иначе квесты провалятся с НПС
...
void QuestToSeaLogin_Add(string groupName)
{
	...
	for(i=0; i<GetAttributesNum(arql); i++)
	{
...
void QuestToSeaLogin_Launch()
{
	...
	//SendMessage(&fader, "ls", FADER_PICTURE0, "interfaces\card_desk.tga");
	SendMessage(&fader, "ls", FADER_PICTURE0, GetLoadingImage_Sea());
...
void SetTimerConditionParamFunction(string _name, string _quest, int _year, int _month, int _day, int _hour, bool _again)
{
    while (_hour > 23)
	{
		_hour -= 23;
    ...
    PChar.quest.(_name).win_condition.l1.date.year  = GetAddingDataYear(_year, _month, _day);
    PChar.quest.(_name).win_condition.l1.date.hour  = _hour;
    PChar.quest.(_name).function                    = _quest;
    if (_again)
    {
        PChar.quest.(_name).again = true;
    }
...
void SetTimerConditionParamEx(string _name, string _quest, int _year, int _month, int _day)
{
	PChar.quest.(_name).win_condition.l1            = "Timer";
    PChar.quest.(_name).win_condition.l1.date.day   = GetAddingDataDay(_year, _month, _day);
    PChar.quest.(_name).win_condition.l1.date.month = GetAddingDataMonth(_year, _month, _day);
...
void SetTimerConditionParamHourEx(string _name, string _quest, int _year, int _month, int _day, int _hour)
{
    while (_hour > 23)
	{
		_hour -= 23;
...
void SetTimerEvent_String(string _event, int _year, int _month, int _day, string _string)
{
	...
	for(int i=1; i<=iMax; i++)
	{
		...
		if(i==iMax)
		{
			Log_TestInfo("SetTimerEvent_String: Свободных прерываний нет, поставить событие невозможно");
			return;
		}
	}
...
// 20.08.09 Метод полностью переписан - вернёт, сколько веса реально будет добавляться при закупке
...
int TransferGoods_StartTransfer(ref rChar, string sColony) // rChar - кому будет ложить товар в трюм, sColony - в какой колонии закупаем
{
	...
	rTreasurer = RefOfficer("treasurer"); // Казначей. Ему даем экспу
...
// belamour (AlexBlade modified) блок квестовых маркеров (СУША)

// Добавление квест-маркера связанный с [questName] над персонажем
// В [questName] рекомендуется указывать id квеста в Судовом журнале
// _Main используется для основных квестов, _Gen для генераторных.
void AddLandQuestMark_Main(ref chr, string questName)
{
    AddLandQuestMark_Main_WithCondition(chr, questName, "");
}

void AddLandQuestMark_Gen(ref chr, string questName)
{
    AddLandQuestMark_Gen_WithCondition(chr, questName, "");
}

// Добавление квест-маркера связанный с [questName] над персонажем
// С дополнительным условием - функцией [sConditionFunc]
// Если условие [sConditionFunc] - true, тогда данная метка будет отображаться над персонажем
// Пустой [sConditionFunc] означает что метка всегда отображается
void AddLandQuestMark_Main_WithCondition(ref chr, string questName, string sConditionFunc)
{
    if (questName == "") return;

    chr.questmarks.(questName) = QUESTMARK_MAIN;

    if (sConditionFunc != "")
    {
        chr.questmarks.(questName).conditionFunc = sConditionFunc;
    }

    SetCurrentQuestMark(chr);
}

void AddLandQuestMark_Gen_WithCondition(ref chr, string questName, string sConditionFunc)
{
    if (questName == "") return;

    chr.questmarks.(questName) = QUESTMARK_GEN;
...
// Удаление квест-маркера связанного с [questName] над персонажем
void RemoveLandQuestMark_Main(ref chr, string questName)
{
    if (questName == "") return;
    if (!CheckAttribute(chr, "questmarks."+questName)) return;

    DeleteAttribute(chr, "questmarks."+questName);

    SetCurrentQuestMark(chr);
}

void RemoveLandQuestMark_Gen(ref chr, string questName)
{
    RemoveLandQuestMark_Main(chr, questName);
}

// Служебный метод для установки метки над персонажем
// Может использоваться во внешнем коде для обновления метки,
// если, например, необходимо повторно проверить условие отображения квест-метки
bool SetCurrentQuestMark(ref chr)
{
    DelLandQuestMark(chr);
    if (!CheckAttribute(&InterfaceStates,"EnabledQuestsMarks")) return false;
    if (InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return false;

    bool hasNewMark = false;
    if (CheckAttribute(chr, "questMarkFantomType"))
    {
        hasNewMark = SetCurrentQuestMark_Fantom(chr, chr.questMarkFantomType);
    }

    if (!CheckAttribute(chr, "questmarks"))
    {
        if (!hasNewMark) DelLandQuestMark(chr);
        return hasNewMark;
    }

    aref qmsRef; makearef(qmsRef, chr.questmarks);

    int questMarksNum = GetAttributesNum(qmsRef);
    if (questMarksNum <= 0)
    {
        if (!hasNewMark) DelLandQuestMark(chr);
        return hasNewMark;
    ...
    string qmModelName;
    string qmCurModelName;
    bool condRes = true;
    for (int i = 0; i < questMarksNum; i++)
    {
        ...
        {
            string condFuncName = curQmRef.conditionFunc;
            condRes = call condFuncName(chr);
        }

        if (condRes)
        {
            qmModelName = qmCurModelName;
            hasNewMark = true;

            if (qmModelName == QUESTMARK_MAIN) // приоритет QUESTMARK_MAIN
            {
                break;
            }
        }
    ...
    if (hasNewMark)
    {
        SetLandQuestMark(chr, qmModelName);
    }

    return hasNewMark;
}

// Служебный метод. Делает метку над персонажем видимой
void SetLandQuestMark(ref chr, string model)
{
...
// Служебный метод. Делает метку над персонажем невидимой
// Не использовать для удаления меток
// Для удаления меток используется метод RemoveLandQuestMark
...
// Флаг игнорирования чекбокса отображения квест-меток в настройках
bool IgnoreQmCheckbox()
{
	if(GetGlobalTutor()) return true;

	return false;
}

// Добавление квест-маркера связанный с [questName] над группой фантомов [sFantomType]
// С дополнительным условием - функцией [sConditionFunc]
// Если условие [sConditionFunc] - true, тогда данная метка будет отображаться над персонажем
// Пустой [sConditionFunc] означает что метка всегда отображается
// Используется для персонажей-фантомов не имеющих постоянного id и/или не имеющих постоянного срока жизни
// Чтобы метки отбражались, персонаж должен быть занесён в группу [sFantomType] используя метод SetLandQuestMarksToFantom
void AddLandQuestMarkToFantoms_Main(string sFantomType, string questName, string sConditionFunc)
{
    if (sFantomType == "") return;
    if (questName == "") return;

    TEV.LandQuestMarks_Fantom.(sFantomType).(questName) = QUESTMARK_MAIN;

    if (sConditionFunc != "")
    {
        TEV.LandQuestMarks_Fantom.(sFantomType).(questName).conditionFunc = sConditionFunc;
    }

    SetCurrentQuestMarkToFantoms(sFantomType);
}

// Удаление квест-маркера связанного с [questName] над группой фантомов [sFantomType]
void RemoveLandQuestMarkToFantoms_Main(string sFantomType, string questName)
{
    if (sFantomType == "") return;
    if (questName == "") return;
    if (!CheckAttribute(&TEV, "LandQuestMarks_Fantom" + "." + sFantomType + "." + questName))
    {
        return;
    }

    DeleteAttribute(&TEV, "LandQuestMarks_Fantom" + "." + sFantomType + "." + questName);

    SetCurrentQuestMarkToFantoms(sFantomType);
}

// Заносит персонажа-фантома в группу [sFantomType]
// Используется в связке с методом AddLandQuestMarkToFantoms
// Примеры групп: Citizen,Commoner - городские жители
// habitue - пьяницы в тавернах
// ItemTrader - уличные торговцы
// JailOff - начальники тюрем в городах
void SetLandQuestMarksToFantom(ref chr, string sFantomType)
{
    if (sFantomType == "") return;
    chr.questMarkFantomType = sFantomType;

    if (!CheckAttribute(&TEV, "LandQuestMarks_Fantom" + "." + sFantomType))
    {
        return;
    }
...
// Удаляет персонажа-фантома из групп фантомов
void RemoveLandQuestMarksToFantom(ref chr)
{
    DeleteAttribute(chr, "questMarkFantomType");

    SetCurrentQuestMark(chr);
}

// Устанавливает метки всем персонажам-фантомам из группы [sFantomType]
void SetCurrentQuestMarkToFantoms(string sFantomType)
{
    ...
    for (int i = GlobalCharacters; i < MAX_CHARACTERS; i++)
    {
        ...
        {
            if (CheckAttribute(chr, "questMarkFantomType") && sFantomType == chr.questMarkFantomType)
            {
                SetCurrentQuestMark(chr);
            }
        }
...
// Устанавливает метку персонажу-фантому из группы [sFantomType]
bool SetCurrentQuestMark_Fantom(ref chr, string sFantomType)
{
    DelLandQuestMark(chr);
    if (!CheckAttribute(&InterfaceStates,"EnabledQuestsMarks")) return false;
    if (InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return false;

    if (sFantomType == "habitue")
    {
        // если не алкаш, метку не ставим
        if (!CheckAttribute(chr, "quest.last_theme") || sti(chr.quest.last_theme) != 0)
        {
            return false;
        }
    }
    ...
    aref qmsRef; makearef(qmsRef, TEV.LandQuestMarks_Fantom.(sFantomType));
...
// belamour (AlexBlade modified) блок квестовых маркеров (ГЛОБАЛКА)
void AddMapQuestMark_Major(string sLocID, string questName, string sConditionFunc)
{
    if (sLocID == "") return;
    if (questName == "") return;
    if (!AddMapQuestMark_CheckLocId(sLocID))
    {
        Trace("Error AddMapQuestMark_Major(): LocID - " + sLocId + " was not found");
        return;
    }

    TEV.MapQuestMarks.(sLocID).(questName) = WDM_QUESTMARK_MAJOR;

    if (sConditionFunc != "")
    {
        TEV.MapQuestMarks.(sLocID).(questName).conditionFunc = sConditionFunc;
    }
}

void AddMapQuestMark_Minor(string sLocID, string questName, string sConditionFunc)
{
    if (sLocID == "") return;
    if (questName == "") return;
    if (!AddMapQuestMark_CheckLocId(sLocID))
    {
        Trace("Error AddMapQuestMark_Minor(): LocID - " + sLocId + " was not found");
        return;
    }

    TEV.MapQuestMarks.(sLocID).(questName) = WDM_QUESTMARK_MINOR;
...
bool AddMapQuestMark_CheckLocId(string sLocID)
{
    return FindColony(sLocID) >= 0 ||
        FindIsland(sLocID) >= 0 ||
        FindLocation(sLocID) >= 0;
}

void RemoveMapQuestMark(string sLocID, string questName)
{
    if (sLocID == "") return;
    if (questName == "") return;

    if (!CheckAttribute(&TEV, "MapQuestMarks" + "." + sLocID + "." + questName))
    {
        return;
    }

    DeleteAttribute(&TEV, "MapQuestMarks" + "." + sLocID + "." + questName);

	// убрать атрибут sLocID, если меток больше нет
	aref arLoc;
	makearef(arLoc, TEV.MapQuestMarks.(sLocID));
	int	Qty = GetAttributesNum(arLoc);
	if (Qty < 1) DeleteAttribute(&TEV, "MapQuestMarks."+sLocID);
}

void SetMapQuestMarks()
{
    DelMapQuestMarks();
    if (!CheckAttribute(&InterfaceStates,"EnabledQuestsMarks")) return;
    if (InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return;
    if (IsEntity(&worldMap)) return;
    // спец. метка для туториала
    if (GetGlobalTutor())
	{
	    SetMapQuestMark("Tortuga_town", WDM_QUESTMARK_MAJOR);
	    return;
	}

    aref locsRef; makearef(locsRef, TEV.MapQuestMarks);

    int locNum = GetAttributesNum(locsRef);

    for (int n = 0; n < locNum; n++)
    {
        string locId = GetAttributeName(GetAttributeN(locsRef, n));
        aref qmsRef; makearef(qmsRef, locsRef.(locId));
        int questMarksNum = GetAttributesNum(qmsRef);
        if (questMarksNum <= 0)
        {
            continue;
        ...
        for (int i = 0; i < questMarksNum; i++)
        {
            ...
            {
                string condFuncName = curQmRef.conditionFunc;
                condRes = call condFuncName(locId);
            }
            ...
            if (condRes)
            {
                ...
                if (qmModelName == WDM_QUESTMARK_MAJOR) // приоритет WDM_QUESTMARK_MAJOR
                {
                    break;
                }
            }
        ...
        if (qmModelName != "")
        {
            SetMapQuestMark(locId, qmModelName);
        }
    }

    CheckMapQuestMarks_FromQuestsCheck(); // ставим метки от прерываний
}

void DelMapQuestMarks()
{
    if (IsEntity(&worldMap)) return;
    aref wdmLabelsRef; makearef(wdmLabelsRef, worldMap.labels);

    int labelsNum = GetAttributesNum(wdmLabelsRef);

    for (int i = 0; i < labelsNum; i++)
    {
        aref attrRef = GetAttributeN(wdmLabelsRef, i);

        if(CheckAttribute(attrRef, "type") &&
            CheckAttribute(attrRef, "heightView"))
        {
            string sType = attrRef.type;
            if (CheckAttribute(&worldMap,"labeltype."+sType+".heightView"))
            {
                attrRef.heightView = worldMap.labeltype.(sType).heightView;
            }
        }

        if(CheckAttribute(attrRef,"icon1"))
            DeleteAttribute(attrRef,"icon1");
        if(CheckAttribute(attrRef,"majorQM"))
            DeleteAttribute(attrRef,"majorQM");
    }
}

void SetMapQuestMark(string sLocId, string model)
{
    if (sLocID == "") return;
    if (model == "") return;
    if (InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return;
    ...
    if(!CheckAttribute(&worldMap,"labels."+sLocId))
    {
        Trace("Error SetMapQuestMark(): Label - " + sLocId + " was not found");
        return;
    }

    if (model == WDM_QUESTMARK_MAJOR)
    {
        worldMap.labels.(sLocId).icon1 = 7;
        worldMap.labels.(sLocId).majorQM = true;
    }
    else
    {
        if(CheckAttribute(&worldMap,"labels."+sLocId+".majorQM"))
        {
            return;
        }
        worldMap.labels.(sLocId).icon1 = 6;
    }
    if (CheckAttribute(&worldMap,"labels."+sLocId+".type") && worldMap.labels.(sLocId).type == "Shore")
    {
        worldMap.labels.(sLocId).heightView = 1000.0;
    }
}

void CheckMapQuestMarks_FromQuestsCheck()
{
	if (InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return;
    ...
    for(int i=0; i<MAX_LOCATIONS; i++)
    {
        ...
        if(loc.type == "seashore" || loc.type == "town") bLocFromSea = true;
        if(!bLocFromSea)
        {
            if(!CheckAttribute(loc,"islandId")) continue;
            if(!CheckAttribute(&worldMap,"labels."+loc.islandId+"")) continue;
            if(bQuestConditionInLoc(loc.id))
            {
                string locId = loc.islandId;
                if (CheckMapQuestMarks_FromQuestsCheck_IsTownLocation(loc))
                {
                    int townLocIdx = FindLocation(loc.townsack+"_town");
                    if (townLocIdx >= 0)
                        locId = Locations[townLocIdx].id;
                }

                AddMapMinorQuestMark_FromQuestCheck(locId);
            }
            continue;
        }
        if(!CheckAttribute(&worldMap,"labels."+loc.id+"")) continue;
        if(CheckAttribute(&worldMap,"labels."+loc.id+".majorQM")) continue;
        if(bQuestConditionInLoc(loc.id))
        {
            AddMapMinorQuestMark_FromQuestCheck(loc.id);
        }
    }
}

void AddMapMinorQuestMark_FromQuestCheck(string sAttr)
{
//    log_testinfo("AddMapMinorQuestMark() - Id "+ sAttr);
//    Trace("AddMapMinorQuestMark() - Id "+ sAttr);
    if(CheckAttribute(&worldMap,"labels."+sAttr+".icon1")) // не затираем метку, если уже стоит
    {
        return;
    }

    SetMapQuestMark(sAttr, WDM_QUESTMARK_MINOR);
}
...
//			Trace("getname "+GetAttributeName(quest)+" locID "+loc_id);
			bConditions = true;
			break;
		}
		if (CheckAttribute(quest, "win_condition.l1") && quest.win_condition.l1 == "locator" && quest.win_condition.l1.Location == loc_id)
...
//			Trace("getname locator "+GetAttributeName(quest)+" locID "+loc_id);
			bConditions = true;
			break;
		}
...
//			Trace("getname l2 "+GetAttributeName(quest)+" locID "+loc_id);
			bConditions = true;
			break;
		}
	}
...
bool CheckMapQuestMarks_FromQuestsCheck_IsTownLocation(ref loc)
{
    return CheckAttribute(loc,"townsack") && loc.townsack != "" &&
        CheckAttribute(loc,"type") && loc.type != "jungle";
}
...
#event_handler("eventChangeOption", "EnableQuestMarks_ChangeOption");
void EnableQuestMarks_ChangeOption()
{
    // Land
    UpdateLandQuestMarks();

    // WorldMap
    // Не требуется
}

void UpdateLandQuestMarks()
{
    if(!CheckAttribute(&InterfaceStates,"EnabledQuestsMarks")) return;
...
bool CheckAlchemyRecipe()
{
	...
	for (i = 0; i < q; i++)
	{
		...
		if (!CheckAttribute(rItem, "ID") || !StrStartsWith(rItem.ID, "recipe_")) continue;

		if (!isMultiObjectKnown(rItem.result))
		{
			if (CurrentInterface == INTERFACE_ALCHEMY)
			{
				...
				i = 0;
				q = GetAttributesNum(arItems);
				LogSound_WithNotify(XI_ConvertString("Already read") + ": " + LowerFirst(GetItemName(rItem.id)), "Notebook_2", "Alchemy");
			}
			else
				return true;
		}
