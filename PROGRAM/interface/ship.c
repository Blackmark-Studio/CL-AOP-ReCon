...
string ShipI_sCachedFacePic;
string ShipI_sCachedShipPic;

int ShipI_idShipsDescr = -1;
int ShipI_idGoodsDescr = -1;
int ShipI_idRPGDescr   = -1;
...
void InitInterface_R(string iniName, ref _chr) // _chr нужно для читового просмотра НПС в море
{
	...
	ShipI_idShipsDescr = LanguageOpenFile("ShipsDescribe.txt");
	ShipI_idGoodsDescr = LanguageOpenFile("GoodsDescribe.txt");
	ShipI_idRPGDescr   = LanguageOpenFile("RPGDescribe.txt");

	ShipI_sCachedFacePic = "";
	...
	XI_RegistryExitKey("CharacterShipMenu");
	///////////
    ...
    GameInterface.TABLE_LIST.hr.td1.str = XI_ConvertString("Goods_in_hold");
	GameInterface.TABLE_LIST.hr.td1.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td2.str = XI_ConvertString("QuantityShort");
	...
	GameInterface.TABLE_LIST.hr.td4.str = XI_ConvertString("Pack");
	GameInterface.TABLE_LIST.hr.td4.scale = 0.9;
	GameInterface.TABLE_LIST.hr.td5.str = XI_ConvertString("PackWeight");
	GameInterface.TABLE_LIST.hr.td5.scale = 0.9;
	GameInterface.TABLE_LIST.select = 0;
	SetCurrentNode("SHIPS_SCROLL");
	//OnShipScrollChange(); // > лишнее, в ProcessFrame() выполняется при запуске интерфейса
	sMessageMode = "";
...
void IDoExit(int exitCode)
{
	...
	if (ShipI_idShipsDescr != -1) { LanguageCloseFile(ShipI_idShipsDescr); ShipI_idShipsDescr = -1; }
	if (ShipI_idGoodsDescr != -1) { LanguageCloseFile(ShipI_idGoodsDescr); ShipI_idGoodsDescr = -1; }
	if (ShipI_idRPGDescr   != -1) { LanguageCloseFile(ShipI_idRPGDescr);   ShipI_idRPGDescr   = -1; }
...
void FillShipsScroll()
{
	...
	string shipName;
	int iShipType, cn;
	// KZ > список групп живёт в глобальном GameInterface - чистим каждый раз
	DeleteAttribute(&GameInterface, "SHIPS_SCROLL.ImagesGroup");
	GameInterface.SHIPS_SCROLL.ImagesGroup.t0 = "BLANK_SHIP2";

	FillShipListSquadronOnly("SHIPS_SCROLL.ImagesGroup", xi_refCharacter);
	...
	GameInterface.SHIPS_SCROLL.NotUsed = 4 - m + 1;
}
...
void ShipI_SetFacePicture(string sPath)
{
	if (ShipI_sCachedFacePic == sPath) return;
	ShipI_sCachedFacePic = sPath;
	SetNewPicture("MAIN_CHARACTER_PICTURE", sPath);
}

void ShipI_SetShipPicture(string sPath)
{
	if (ShipI_sCachedShipPic == sPath) return;
	ShipI_sCachedShipPic = sPath;
	SetNewPicture("SHIP_BIG_PICTURE", sPath);
}

void OnShipScrollChange()
{
	ShipI_SetFacePicture("interfaces\portraits\256\face_" + xi_refCharacter.FaceId + ".tga");
	SetFormatedText("MAIN_CHARACTER_NAME", GetFullNameTitulForm(xi_refCharacter));
	SetFormatedText("MAIN_CHARACTER_RANK",xi_refCharacter.Rank);
	...
	if (iShip != SHIP_NOTUSED)
	{
		...
		ShipI_SetShipPicture("interfaces\ships\" + sShip + ".tga");
		...
		if (GetCompanionQuantity(pchar) > 1) // больше 1 ГГ
		{
			...
			sText = XI_ConvertString("Rum_on_squadron") + " " + XI_ConvertString("ForShip") + " ";
			iRum = CalculateRum();
		...
		if (GetRemovable(xi_refCharacter)) // считаем только своих, а то вских сопровождаемых кормить!!!
		{
		    ...
		    SetFormatedText("MONEY_SHIP", XI_ConvertString("Pay_for_ship") + " " + NewStr() + FindMoneyString(GetSalaryForShip(xi_refCharacter)));
		}
		int iCrewQty = GetCrewQuantity(xi_refCharacter);
		SetFormatedText("CREW_QTY", "" + iCrewQty);
		if (iCrewQty > GetOptCrewQuantity(xi_refCharacter) || iCrewQty < GetMinCrewQuantity(xi_refCharacter))
		{
		...
		int iMorale = sti(xi_refCharacter.Ship.crew.morale);
		SetNewGroupPicture("CREW_MORALE_PIC", "MORALE_SMALL", GetMoraleGroupPicture(iMorale));
		SetFormatedText("CREW_MORALE_TEXT", XI_ConvertString("CrewMorale") + ": " + XI_ConvertString(GetMoraleName(iMorale)));
		if (iMorale < MORALE_MAX && iCrewQty > 0)
		{
    	...
    	GameInterface.StatusLine.BAR_CrewMoral.Value = iMorale;
	...
	{
		ShipI_SetShipPicture("interfaces\blank_ship2.tga");
...
void ShowInfoWindow()
{
	...
	switch (sCurrentNode)
	{
		...
		case "TABLE_OTHER":
			if (!CheckAttribute(&GameInterface, CurTable + "." + CurRow + ".UserData.ID"))
			{
				bShowHint = false;
			}
			else
			{
			sOtherID = GameInterface.(CurTable).(CurRow).UserData.ID;
			sHeader = XI_ConvertString(sOtherID);
		    sText1  = GetConvertStr(sOtherID, "ShipsDescribe.txt");
		    if (sOtherID == "CannonType" && sti(xi_refCharacter.Ship.Cannons.Type) != CANNON_TYPE_NONECANNON)
		    {
		    ...
		    if (sOtherID == "Crew" && sti(xi_refCharacter.ship.type) != SHIP_NOTUSED)
			{
				sText2 = XI_ConvertString("Max_crew_text1");
				sText2 = sText2 + NewStr() + XI_ConvertString("Max_crew_text2") + " " + GetMaxCrewQuantity(xi_refCharacter);
			}
			// процент ремонта
			if (sOtherID == "Hull" && sti(xi_refCharacter.ship.type) != SHIP_NOTUSED)
			{
                if (GetHullPercent(xi_refCharacter)<100.0 )
                {
					sText2 = XI_ConvertString("RepairShipShort") + " " + FloatToString(GetHullRepairDay(xi_refCharacter, false),2) + XI_ConvertString("PortmanMessageInfo_12") + makeint(GetHullRepairDay(xi_refCharacter, true)) + " " + XI_ConvertString("pcs") + " " + LowerFirst(XI_ConvertString("PlanksGen"));
				}
				sText3 = xiStr("Hull") + ": " + FloatToString(GetHullPercent(xi_refCharacter), 1)  + " %";
			}
			if (sOtherID == "Sails" && sti(xi_refCharacter.ship.type) != SHIP_NOTUSED)
			{
                if (GetSailPercent(xi_refCharacter)<100.0 )
                {
					sText2 = XI_ConvertString("RepairShipShort") + " " + FloatToString(GetSailRepairDay(xi_refCharacter, false),2) + XI_ConvertString("PortmanMessageInfo_12") + makeint(GetSailRepairDay(xi_refCharacter, true))+ " " + XI_ConvertString("pcs") + " " +  LowerFirst(XI_ConvertString("SailclothGen"));
				}
				sText3 = xiStr("Sails") + ": " + FloatToString(GetSailPercent(xi_refCharacter), 1) + " %";
			}
			// трюм
			if (sOtherID == "Capacity" && sti(xi_refCharacter.ship.type) != SHIP_NOTUSED)
			{
				sText3 = XI_ConvertString("Occupied") + ": " + FloatToString((stf(GetCargoLoad(xi_refCharacter))  /  stf(GetCargoMaxSpace(xi_refCharacter))) * 100.0, 1)+ " %";
			}
			...
			string sTemp = "";
			string sAttr = "";
			if(CheckAttribute(refBaseShip,"atlaua_bonus"))
			{
				switch(sOtherID)
				{
					case "Hull":		sAttr = "HP";			break;
					case "Crew":		sAttr = "MaxCrew";		break;
					case "Capacity":	sAttr = "Capacity";		break;
					case "Speed":		sAttr = "SpeedRate";	break;
					case "Maneuver":	sAttr = "TurnRate";		break;
				}
				if(sAttr != "" && CheckAttribute(refBaseShip,"atlaua_bonus."+sAttr) && refBaseShip.atlaua_bonus.(sAttr) != "0")
				{
					switch(refBaseShip.atlaua_bonus.(sAttr))
					{
						case "2":		sTemp = " " + XI_ConvertString("twice");	break;
						case "full":	sTemp = " " + XI_ConvertString("thrice");	break;
					}
					sText4 = XI_ConvertString("PortmanMessageInfo_13")+sTemp+XI_ConvertString("PortmanMessageInfo_14");
				}
			}
			}
		break;
	...
	if (bShowHint)
	{
		CreateTooltip("#" + sHeader, sText1, argb(255,255,255,255), sText2, argb(255,255,192,192), sText3, argb(255,192,255,192), sText4, argb(255,139,0,255), sPicture, sGroup, sGroupPicture, 64, 64);	// evganat - пасха
	}
}
...
void ShowRPGHint()
{
    ...
    SetFormatedText("OFFICER_NAME", GetFullNameTitulForm(xi_refCharacter));
...
void FillGoodsTable()
{
    ...
    for (i = 0; i< GOODS_QUANTITY; i++)
	{
        row = "tr" + n;
		sGood = Goods[i].name;
		qty = sti(xi_refCharacter.Ship.Cargo.Goods.(sGood));
		...
		GameInterface.TABLE_LIST.(row).td1.icon.width = 28;
		GameInterface.TABLE_LIST.(row).td1.icon.height = 28;
...
void DropGoodsToSeaFromInterface(int iGoodIndex, int iQuantity)
{
	...
	{		
		...
		if (bSeaActive && !bAbordageStarted)  // море, но не каюта
		{
			...
			RecalculateCargoLoad(xi_refCharacter);
	        xi_refCharacter.Tmp.SpeedRecall = 0; // чтоб пересчитались скорость и манёвр
...
void ShipChange()
{
	...
	if (xi_refCharacter.id != pchar.id && sti(pchar.ship.type) != SHIP_NOTUSED)
	{
		...
		if (bDisableMapEnter || bOk || and(chrDisableReloadToLocation, !GetGlobalTutor()))
		{
			SetFormatedText("REMOVE_WINDOW_TEXT", XI_ConvertString("ShipChangeMsg_4"));
			SetSelectable("REMOVE_ACCEPT_OFFICER", false);	
		}
		if (bSeaActive && GetShipEscape())
		{
			SetFormatedText("REMOVE_WINDOW_TEXT", XI_ConvertString("ShipChangeMsg_5"));
			SetSelectable("REMOVE_ACCEPT_OFFICER", true);	
		}
		if (!GetRemovable(xi_refCharacter))
		{
...
void ShowMoraleMessage()
{
	...
	SetFormatedText("REMOVE_WINDOW_TEXT", XI_ConvertString("RaiseMoraleMoney") + FindMoneyString(sum));
...
void CannonsMenuRefresh()
{
	...
	string goodName, goodsDescr;

	if (idx != -1)
	{
		goodName = goods[idx].name;
		goodsDescr = XI_ConvertString("InstalledCannon") + ": " + XI_ConvertString(goodName) + "\n\n" + GetAssembledString(GetConvertStr(goodName+"_descr", "GoodsDescribe.txt"), &Goods[idx]);
...
void FillCannonsTable()
{
	...
	GameInterface.CANNONS_TABLE.hr.td1.str = XI_ConvertString("CannonsInHold");
	GameInterface.CANNONS_TABLE.hr.td1.scale = 0.8;
	GameInterface.CANNONS_TABLE.hr.td2.str = XI_ConvertString("QuantityShort");
	GameInterface.CANNONS_TABLE.hr.td2.scale = 0.8;
	GameInterface.CANNONS_TABLE.hr.td3.str = XI_ConvertString("ShortDistance");
	GameInterface.CANNONS_TABLE.hr.td3.scale = 0.8;
	GameInterface.CANNONS_TABLE.hr.td4.str = XI_ConvertString("Damage");
	GameInterface.CANNONS_TABLE.hr.td4.scale = 0.8;
	GameInterface.CANNONS_TABLE.hr.td5.str = XI_ConvertString("Time:");
	...
	{
	    int iMaxCaliber = GetMaximumCaliber(xi_refCharacter);
	    for (i = 0; i< GOODS_QUANTITY; i++)
		{
			if (!CheckAttribute(&Goods[i], "type")) continue;
			if (sti(Goods[i].type) != TRADE_TYPE_CANNONS) continue;
			if (GetCannonCaliber(sti(Goods[i].CannonIdx)) > iMaxCaliber) continue;
...
void ShowPartitionWindow()
{
	...
	str = XI_ConvertString("Current Date") + GetDateString() + " " + GetTimeString() + NewStr();
	if(CheckAttribute(pchar, "paymentdate"))
	{
		str += XI_ConvertString("ShareLootDate") + pchar.paymentdate + NewStr();
	}	
	str += sTitul + " " + GetFullNameTitulForm(pchar) + NewStr() + XI_ConvertString("Rank") + ": " + sti(pchar.rank) + NewStr();
	str += XI_ConvertString("m_Complexity") + ": " + GetLevelComplexity(MOD_SKILL_ENEMY_RATE) + NewStr() +
			XI_ConvertString("OurMoney") + FindMoneyString(sti(pchar.money)) + NewStr() + "*****" + NewStr();
    str += XI_ConvertString("ShareLootForMonth") + NewStr() + XI_ConvertString("ShareLootCapitan") + GetPartitionAmount("Partition.MonthPart.Hero") + NewStr() +
		  XI_ConvertString("ShareLootOfficers") + GetPartitionAmount("Partition.MonthPart.Officers") + NewStr() +
		  XI_ConvertString("ShareLootSailors") + GetPartitionAmount("Partition.MonthPart.Crew") + NewStr() + // + "-----" + NewStr() +
		  XI_ConvertString("ShareLootDebt") + GetPartitionAmount("Partition.MonthPart") + NewStr();
	str += "*****" + NewStr() + XI_ConvertString("ShareLootDebtForMonth") + GetPartitionAmount("CrewPayment");
...
{
	int sum = 0;
	//очевидно что этот иф не работал
	//if (GetPartitionAmount("Partition.MonthPart") > 0 || GetPartitionAmount("CrewPayment") > 0)
...
void OnHeaderClick()
{
	...
	if (sControl == "TABLE_LIST") SortItemsTable(iColumn, false, sControl);
}
