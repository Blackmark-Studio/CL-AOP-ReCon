...
void SetVariable()
{
	...
	//Скрывать количество нераспределённых очков, если уже всё собрал
	...
	if (bBettaTestMode)
    {
        CreateString(true,"healthCur1",
                     ...
                     "INTERFACE_ULTRASMALL",COLOR_NORMAL,10,567, SCRIPT_ALIGN_LEFT,1.0);
    }
...
void ProcessFrame()
{
	...
	if (CheckAttribute(&GameInterface, "PASSENGERSLIST") && sti(GameInterface.PASSENGERSLIST.current) != nCurScrollOfficerNum && GetCurrentNode() == "PASSENGERSLIST")
	{
...
void SetButtonsState()
{
	...
	string sChar = GameInterface.CHARACTERS_SCROLL.(attributeName).character;

	HideSkillChanger();

	if (sChar != "0")
		xi_refCharacter = &characters[sti(sChar)];
	else
        xi_refCharacter = pchar;
...
void ShowInfoWindow()
{
	...
	switch (sCurrentNode)
	{
		case "CHARACTERS_SCROLL":
            ...
            if(CheckAttribute(GetCharacter(iCharacter), "Capellan"))
				 sHeader = XI_ConvertString("capellan");
			else sHeader = XI_ConvertString("passengership");
			switch(nChooseNum)
			{
				case 0:
			...
			if(iCharacter != 0)
			{
				...
				if (iItem > 0)
				{
				    sText2 = XI_ConvertString("delivery") + " " + FindMoneyString(iItem) + " " + XI_ConvertString("per month");
				}
				if(FindFellowtravellers(pchar,rchr) != FELLOWTRAVEL_COMPANION)
				{
					...
					{
                        sHeader = GetRPGText("Prisoner_hint");
            ...
            sText3 = XI_ConvertString("ChatacterInderfaceDescr_8");
            if (bBettaTestMode)
            {
		...
		case "TABLE_SKILL_1":
		    ...
		    sText1  = GetRPGText(GameInterface.(CurTable).(CurRow).UserData.ID + "_hint");
			if (GameInterface.(CurTable).(CurRow).UserData.ID == "Sneak" && xi_refCharacter.id == pchar.id)
			{
				int chance = round_up(makefloat((GetSneakChance() / 120) * 100));
				...
				reColor = true;
				sText2 = XI_ConvertString("ChatacterInderfaceDescr_9") + XI_ConvertString("ChatacterInderfaceDescr_10");
				sText3 = XI_ConvertString("ChatacterInderfaceDescr_11") + chance + "%";
			}
		break;
		case "TABLE_SKILL_2":
	...
	if (reColor)
		CreateTooltip("#" + sHeader, sText1, argb(255,255,255,255), sText2, argb(255,192,255,192), sText3, argb(255,255,255,105), "", argb(255,255,255,255), sPicture, sGroup, sGroupPicture, 64, 64);
	else
...
void SetSkillArrows()
{
   	bool ok = CurTable == "TABLE_SKILL_1" || CurTable == "TABLE_SKILL_2";
	if (sti(xi_refCharacter.skill.freeskill) > 0 && ok)
	{
	    if (GetSkillValue(xi_refCharacter, SKILL_TYPE, GameInterface.(CurTable).(CurRow).UserData.ID) > 1)
        {
			...
			{
				SendMessage(&GameInterface,"lsllllll", MSG_INTERFACE_MSG_TO_NODE,"B_SKILLDown",0, 682, 93 + 30 * (iSelected - 1), 714, 93 + 28 + 30 * (iSelected - 1), 0);
			}
			else
			{
				SendMessage(&GameInterface,"lsllllll", MSG_INTERFACE_MSG_TO_NODE,"B_SKILLDown",0, 682, 339 + 30 * (iSelected - 1), 714, 339 + 28 + 30 * (iSelected - 1), 0);
			}
        ...
        if (GetSkillValue(xi_refCharacter, SKILL_TYPE, GameInterface.(CurTable).(CurRow).UserData.ID) < SKILL_MAX)
        {
            if (CurTable == "TABLE_SKILL_1")
			{
				SendMessage(&GameInterface,"lsllllll", MSG_INTERFACE_MSG_TO_NODE,"B_SKILLUP",0, 720, 93 + 30 * (iSelected - 1), 752, 93 + 28 + 30 * (iSelected - 1), 0);
			}
			else
			{
				SendMessage(&GameInterface,"lsllllll", MSG_INTERFACE_MSG_TO_NODE,"B_SKILLUP",0, 720, 339 + 30 * (iSelected - 1), 752, 339 + 28 + 30 * (iSelected - 1), 0);
			}
...
void SetControlsTabMode(int nMode)
{
	...
	switch (nMode)
	{
		case 1: //
			...
			if(sPerkMode == "traits" || bAllSelf)	break;	// evganat - черты
			// порог опыта
		...
		case 2:
			...
			if(sPerkMode == "traits" || bAllShip)	break;	// evganat - черты
			// порог опыта
...
void FillPassengerScroll()
{
	...
	for(i=0; i<nListSize; i++)
	{
		...
		if (_curCharIdx!=-1)
		{
			...
			{
		        ok2 = true; //обновляем дабы не сейвилось за первую итерацию
                // совместители должностей -->
                howWork = 1;
                if (CheckCharacterPerk(chr, "ByWorker"))
                {
                    howWork = 2;
                }
                if (CheckCharacterPerk(chr, "ByWorker2"))
                {
                    howWork = 3;
                }
                ok = !CheckAttribute(chr, "isfree") || sti(chr.isfree) < howWork;
                PsgAttrName = GetOfficerTypeByNum(nCurScrollNum);
				// совместители должностей <--

				if (ok && !CheckAttribute(chr, PsgAttrName))
				{
					ok = CheckAttribute(chr, "AllowedPosts") && HasSubStr(chr.AllowedPosts, PsgAttrName);
					...
					if (howWork > 1)
					{
						...
						{
							if (GetAttributeQty(chr, "", "navigator,boatswain,cannoner,doctor,treasurer,carpenter") >= 1)
								ok2 = false;
						}
					}
					
					if (!CheckAttribute(chr, "AllowedPosts") || ok)
					{
						if (!ok2 && PsgAttrName != "fighter")
							continue;
...
void AcceptAddOfficer()
{
    ...
    if (CheckAttribute(&GameInterface, "PASSENGERSLIST."+attributeName2 + ".character"))
    {
		...
		if (!CheckAttribute(rChar, "isfree"))
		{
			rChar.isfree = 1;
		}
		else
		{
		    rChar.isfree = sti(rChar.isfree) + 1; // совместители
		}
		bOk = (rChar.location != pchar.location);  // ниже локация перебивается на ГГ
    	...
    	rChar.(attributeName2) = true; // совместитель дожности
    	//SetCharacterTask_FollowCharacter(rChar, PChar);
    	if (bNeedFollow) // только для офов
    	{
	    	//if (rChar.location.group == "sit")
	    	...
	    	{   //  пусть всегда будут появляться
				LAi_PlaceCharInTavern(rChar);
	    	}
			if (IsEntity(loadedLocation) && rChar.id == "Douglas" && loadedLocation.id == "PortSpein_houseH3")
	    	{   //  Rosarak: для Хемфри Дугласа
...
{
	int perksQ, i, n, iColor = argb(255,255,255,255);
	string perkName, row, icoGroup;
	aref arPerksRoot, arChrPerks;
	...
	if(sPerkMode == "perks")
	{
		...
		for(i=0; i<perksQ; i++)
		{
			...
			if (CheckAttribute(arPerksRoot, perkName + ".BaseType") && arPerksRoot.(perkName).BaseType == _type)
			{
				...
				GameInterface.TABLE_PERKS.(row).td2.color = iColor;
...
{
	if(CurRow == "") return;
	if(!CheckAttribute(&GameInterface, CurTable+"."+CurRow)) return;	// evganat - черты
	bool ok = true;
...
void AcceptPerk()
{
	string sTemp;
    string  perkName = GameInterface.(CurTable).(CurRow).UserData.ID;
	if(!CheckAttribute(&ChrPerksList, "list."+perkName+".Free"))	// EvgAnat - дикие карибы
	{
	    if (GameInterface.(CurTable).(CurRow).UserData.Type == "self")
    	{
