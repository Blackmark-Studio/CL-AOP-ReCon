...
//HardCoffee что бы заново не подбегать к сундуку
void CharacterStillInLocator()
{
	aref loc = GetEventData();
	aref chr = GetEventData();
	string group = GetEventData();
	string locator = GetEventData();
	SetEventHandler("AgainInLocator", "chrCharacterEntryToLocator", 0);
	event("AgainInLocator", "iiss", loc, chr, group, locator);
	DelEventHandler("Location_CharacterInLocator", "CharacterStillInLocator");
	DelEventHandler("AgainInLocator", "chrCharacterEntryToLocator");
}
...
void chrCharacterEntryToLocator()
{
	...
	{
	case "reload":
		//HardCoffee Если игрок пришвартовал корабль вместо лодки, то грузиться из локатора boat нельзя -->
		if ("boat" == locator && HasSubStr(loadedlocation.id, "town"))
		{
			for (i = 0; i < locNumShips; i++)
			{
				if (iShips[i] < 0) continue;
				rChr = GetCharacter(&iShips[i]);
				if ("BoatChar" == rChr.id) goto linkBoat;
			}
			break;
		}
		linkBoat:
...
/*	case "camdetector":
	...
	case "animals":
		if (!SendMessage(&Sound, "ll", MSG_SOUND_IS_PLAYING, CharVoice) && HasSubStr(locator, "parrot"))
		{
			Log_SetActiveAction("Parrot");
		}
		break;
	}
...
void chrCharacterInLocator()
{
	...
	if (!SendMessage(&Sound, "ll", MSG_SOUND_IS_PLAYING, CharVoice) && locator == "parrot")
	{
		Log_SetActiveAction("Parrot");
	}
	//HardCoffee чтобы повторно не вбегать в локатор можно каждую секунду чекать локаторы
	//event("Location_CharacterEntryToLocator", "iiss", loc, chr, group, locator);
}
...
ref funcGetWeaponID()
{
	...
	if (iTemp != -1 && CheckAttribute(&characters[iTemp], "equip.blade"))
	{
		string bladeId = characters[iTemp].equip.blade;
		if (HasSubStr(bladeId, "topor")) g_strRetParam = "topor";
	}	
