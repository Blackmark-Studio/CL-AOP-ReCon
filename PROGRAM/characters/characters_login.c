...
void LoginCharacter(aref chr, string locID)
{
	if(LAi_CharacterLogin(chr, locID))
	{
		...
		else Trace("Can't create character: " + chr.id);
		
		if(isNoCreated) LAi_CharacterLogoff(chr);
	}
}
...
{
	return ChangeCharacterAddressGroupEx(character, location_id, group, locator, false);
}
//HardCoffee заставить нпс переместится в локацию, даже если он уже в ней находится
//Используется в случае, если нпс был присвоен атрибут location до фактического его перемещения
bool ChangeCharacterAddressGroupEx(ref character, string location_id, string group, string locator, bool bForced)
{
	//Trace("Change character address id = " + character.id + "; location = " + location_id + "; group = " + group + "; locator = " + locator);
	//если уже находимcя на месте то зачем куда-то переезжать?
    if (!bForced && CheckAttribute(character, "location.locator") && character.location == location_id && character.location.group == group && character.location.locator == locator)
        return true;

    //если ГГ сидит и его перемещают, значит пора вставать
    if (CheckAttribute(&TEV, "begin_sit") && GetMainCharacterIndex() == sti(character.index))
    {
        ResetParamsAfterSit();
    }
	...
	//Если перемещаемся внутри одной локации, то просто телепортимся
	...
	{
		if (!bForced && location_id == character.location && IsEntity(character)) //boal fix 26/06/05
		{
