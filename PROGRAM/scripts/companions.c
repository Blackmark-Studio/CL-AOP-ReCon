...
void PlaceCompanionCloneNearMChr(int _index, bool _isCampus)
{
	...
	LAi_SetWarriorTypeNoGroup(sld); //HardCoffee это для того, чтобы они могли драться с пленными
	...
	else PlaceCharacter(sld, "rld", PChar.location);
	LAi_tmpl_goto_SetLocator(sld, "rld", LAi_FindFreeRandomLocator("rld"), -1.0);
...
void CompanionSaveTasks()
{
	...
	for (i = 0; i < COMPANION_MAX; i++)
	{
		...
		if (CheckAttribute(chr, "Tasks.Clone"))
		{
			...
			ChangeCharacterAddress(sld, "none", ""); //HardCoffee - меняю, чтобы компаньоны корректно уходили из каюты во время драк
			//sld.location = "none";
...
void ThisCompanionSaveTasks() //HardCoffee выгнать из каюты конкретного компаньона
{
	int idx;
	ref rReal, rClone;
	aref arReal, arClone;
	//Цикл с чтением атрибута SaveTasks, вместо непосредственной передачи ссылки из ивента
	//из за того, что некоторые могут застревать, если выгонять компаньонов одного за другим
	for (int i = 0; i < COMPANION_MAX; i++)
	{
		idx = GetCompanionIndex(PChar, i);

		if (idx < 0) continue;
		rReal = GetCharacter(idx);
		if (!CheckAttribute(rReal, "Tasks.Clone")) continue;
		rClone = GetCharacter(sti(rReal.Tasks.Clone));
		if (!CheckAttribute(rClone, "RealCompanionIdx") || sti(rClone.RealCompanionIdx) < 0)
		{
			trace("companions.c ThisCompanionSaveTasks method can't find RealCompanionIdx");
			CompanionSaveTasks();
			return;
		}
		if (!CheckAttribute(rClone, "IsCompanionClone.SaveTasks")) continue;
		DeleteAttribute(rClone, "IsCompanionClone.SaveTasks");

		makearef(arReal, rReal.Tasks);
		makearef(arClone, rClone.Tasks);
		CopyAttributes(arReal, arClone);
		ChangeCharacterAddress(rClone, "none", "");
		DeleteAttribute(rReal, "Tasks.Clone");
	}
}
//navy <--
