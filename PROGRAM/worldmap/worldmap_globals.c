...
bool wdmIsEncounterEnemy(int index)
{
	...
	int iNation = PIRATE;
	bool isProfessional = (IsCharacterPerkOn(pchar, "SeaDogProfessional")) && (GetPcharSquadronShipClass(0, 0) <= 4);
	
	if (CheckAttribute(rEnc, "nation"))
		iNation = sti(rEnc.Nation);
	
	if (isProfessional || (GetNationRelation2MainCharacter(iNation) != RELATION_ENEMY))
		return false;
	...
	int iSquadronBestClass = GetPcharSquadronShipClass(0, 0);					// лучший класс корабля в эскадре ГГ > TODO
	...
	bool smallPir = iEncType == ENCOUNTER_TYPE_PIRATE_SMALL;
	bool mediumPir = (smallPir) || (iEncType == ENCOUNTER_TYPE_PIRATE_MEDIUM);
	bool largePir = (mediumPir) || (iEncType == ENCOUNTER_TYPE_PIRATE_LARGE);
	...
	if (GetCharacterShipClass(pchar) <= 3 && bSquadronType && smallPir)
		return false;
	...
	if (GetCharacterShipClass(pchar) <= 4 && iSquadronQuantity > 1 && iSquadronAverageClass <= 4 && bSquadronType && mediumPir)
		return false;
	
	// если ГГ на <= 2 классе и эскадра преимущественно боевая, то энкаунтеры средних пиратов не нападают
	if (GetCharacterShipClass(pchar) <= 2 && bSquadronType && mediumPir)
		return false;
	...
	if (GetCharacterShipClass(pchar) <= 3 && iSquadronQuantity > 1 && iSquadronAverageClass <= 3 && bSquadronType && mediumPir)
		return false;
	...
	if (GetCharacterShipClass(pchar) < 2 && iSquadronQuantity > 2 && iSquadronAverageClass <= 2 && bSquadronType && largePir)
		return false;
