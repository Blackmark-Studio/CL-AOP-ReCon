...
void InitColonies()
{
	...
	for (i=0; i<MAX_COLONIES; i++)
	{
		...
		// TODO del переносить нужное наверх, остальное тереть -->
		Colonies[i].money = 0;
		Colonies[i].reload_enable = true;
		Colonies[i].visible = true;
		Colonies[i].capturetime = "";  // TODO del
		Colonies[i].capture_day = ""; // TODO del
		Colonies[i].capture_month = ""; // TODO del
		Colonies[i].capture_year = "";  // TODO del
		...
		Colonies[i].capture_flag = "0"; // TODO del
		Colonies[i].agressor = "-1";
		Colonies[i].loyality = "1";
		Colonies[i].disease = "0";
		Colonies[i].disease.time = "0";

		Colonies[i].num = 1; //Используется для номеров в группе IslandShips; TODO: когда у нас будет несколько портов на острове, надо всё переименовать
		...
		colonies[i].jail = 1; //upd: Рандом перенесен в генерацию нач-тюрем
	...
	/////////////////////////////////////////////////////////////////////
	    ...
	    colonies[n].jail = 0; // тюрьма Порт-Ройал всегда образцовая. нужно по квестам.
		colonies[n].mayak = 3;
	...
	///////////////////////////////////////////////////////////////////// 
		...
		colonies[n].FortValue = 95; // homo
		colonies[n].notCaptured = true; //не давать захватывать город себе
	...
	///////////////////////////////////////////////////////////////////// 
		...
		//colonies[n].Default.BoardLocation = "LaVega_ExitTown"; //Появится по квесту
		colonies[n].HasNoFort = true;
	...
	/////////////////////////////////////////////////////////////////////
		...
		Colonies[n].num = 1;
		colonies[n].map.x = 1555;
		colonies[n].map.y = 505;
		colonies[n].StoreNum   = Providencia_STORE;
		colonies[n].from_sea   = "Providencia_town";
		colonies[n].Default.BoardLocation = "Providencia_ExitTown";
	...
	/////////////////////////////////////////////////////////////////////
		...
		colonies[n].from_sea   = "Panama_town";    // нет HasNoFort чтоб грузился форт, захват только с суши при захваченном Портобело, в магазах шлют
		colonies[n].Default.BoardLocation = "Panama_ExitTown";
		colonies[n].notCaptured = true; //не давать захватывать город себе
		// FortValue  нет, чтоб не брать осадами
		// для генератора нужно исключать, но пока не ясно colonies[n].HasNoFort = true
