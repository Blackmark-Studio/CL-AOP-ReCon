...
int Fantom_GetShipTypeExt(int iClassMin, int iClassMax, string sShipType, string sGroupName, string sFantomType, int iEncounterType, int iNation)
{
	...
	for (i = SHIP_BILANCETTA; i <= SHIP_MANOWAR; i++)  //энкаунтеры от баркаса до мановара, квестовые корабли отдельно
	{
...
void Fantom_SetCannons(ref rFantom, string sFantomType)
{
	...
	switch(iCaliber)
	{
		...
		case 20: iCaliber = 5; break;
		case 24: iCaliber = 6; break;
		case 32: iCaliber = 7; break;
		case 36: iCaliber = 8; break;
		case 42: iCaliber = 9; break;
	}
	if(iCaliber > 0)	iCaliber = iCaliber - rand(1);
	else				iCaliber = 0;
	switch(iCaliber)
	{
		...
		case 5: iCaliber = 20; break;
		case 6: iCaliber = 24; break;
	...
	if (iCaliber == 42 || iCaliber == 36 || iCaliber == 20 || iCaliber == 6 || iCaliber == 3)
		sCannonType = "cannon"; //для калибров без кулеврин
...
void Fantom_SetGoods(ref rFantom, string sFantomType)
{
	...
	int iFinish = GOOD_PINCTADA - iStart - 1; // исключая пушки
	...
	switch (sFantomType)
    {
		...
		case "trade":
			if (CheckAttribute(rFantom, "RealEncounterType") && sti(rFantom.RealEncounterType) == ENCOUNTER_TYPE_ESCORT_LARGE)
			{
				...
				{
    					iFinish = GOOD_PINCTADA - iStart - 1;
				}
				else 
				{
