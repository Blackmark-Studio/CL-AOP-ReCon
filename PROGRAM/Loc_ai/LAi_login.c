...
bool LAi_CharacterLogin(aref chr, string locID)
{
	...
	{
		chr.chr_ai.energy = LAI_DEFAULT_ENERGY; // TODO to max
	}*/
...
void LAi_CharacterPostLogin(ref location)
{
	if (bSeaActive && !LAi_IsBoardingProcess()) //HardCoffee заселим корабль
	{
		...
		switch(location.id)
		{
			...
			case "My_Orlop": SetSailorsInGunDeck(location); break;
		}
	}
...
bool LAi_CharacterLogoff(aref chr)
{
	...
	//chr.location = "none";
	if(LAi_CheckCharacter(chr, "LAi_CharacterLogoff") == false || !CheckAttribute(chr, "chr_ai.type")) return false;
