...
//сломать половину мачт корабля. Если thanHalf = true то сломать все что больше "середины". Иначе - меньше.
void BrokeMasts(ref char, bool thanHalf)
{
    aref aMasts, aMast;
    int start, end;

	makearef(aMasts, char.ship.masts);
	int mastsSize = GetAttributesNum(aMasts);
	if (thanHalf)
	{
	    start = mastsSize / 2;
	    end = mastsSize;
