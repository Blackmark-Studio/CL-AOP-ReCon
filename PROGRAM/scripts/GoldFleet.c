...
{
    // апаем ЗФ ZhilyaevDm
    // теперь будет Мановар, 2 x Линейный, Военный + 4 Тяжелых галеона
    ref sld, chref;
    int iChar = NPC_GenerateCharacter("Head_of_Gold_Squadron", "off_spa_6", "man", "man", 5, SPAIN, 31, true);
    makeref(chref, Characters[iChar]);

    chref.ship.type = GenerateShipExt(SHIP_MANOWAR, 1, chref);
    chref.ship.cannons.type = CANNON_TYPE_CANNON_LBS36;
    AddCrewMorale(chref, 100);
	...
	chref.dialog.filename = "Capitan_dialog.c"; // homo 20/01/07
    ...
    int nwar = 3;
    int ntrade = 4;
    GoldCapNum = nwar + ntrade;

    for (int k = 1; k <= (nwar + ntrade); k++)
    {
        ...
        if (k <= nwar)
        {
            ...
            {
            case 1:
                sld.ship.type = GenerateShipExt(SHIP_WARSHIP, 1, sld);
                break;
            case 2:
                sld.ship.type = GenerateShipExt(SHIP_WARSHIP, 1, sld);
                break;
            case 3:
                sld.ship.type = GenerateShipExt(SHIP_LINESHIP, 1, sld);
                break;
            }
        ...
        {
            sld.Ship.Mode = "trade";
            sld.ship.type = GenerateShipExt(SHIP_GALEON_H, 1, sld);
        ...
        sld.dialog.filename = "Capitan_dialog.c"; // homo 20/01/07
...
void LeaveGoldleet(string temp)
{
    ...
    if ( isGoldFleet )
    {
        if (NumDeadCapitan() > 0)
        {
            ...
            Achievment_Set(ACH_Urca_de_Lima);
        }
        else
        {
            // конвой не тронули
...
{
	if (Colonies[FindColony("Havana")].nation == SPAIN && Colonies[FindColony("PortoBello")].nation == SPAIN && !CheckAttribute(&Colonies[FindColony("Havana")], "Siege") && !CheckAttribute(&Colonies[FindColony("PortoBello")], "Siege"))
	{
